...
2. Dify Brain(LLM)이 자연어/기획서/녹화를 14대 확장 DSL(고급 상호작용 및 네트워크 모킹 포함)로 변환한다.
...
| 7 | `ZeroTouch-QA` | 자연어 시나리오 기반으로 웹 애플리케이션의 E2E 테스트를 자율 수행한다. | Section 5.8 |
...
| 모듈 | 역할 |
|---|---|
| `shared/zero_touch_qa/__main__.py` | CLI 엔트리포인트. `--mode chat\|doc\|convert\|execute` 4대 모드 지원 |
| `shared/zero_touch_qa/config.py` | 환경변수를 Config dataclass로 일괄 관리 |
| `shared/zero_touch_qa/dify_client.py` | Dify Chatflow API 통신 (시나리오 생성 + 치유 요청) |
| `shared/zero_touch_qa/locator_resolver.py` | 7단계 시맨틱 DOM 탐색 엔진 |
| `shared/zero_touch_qa/local_healer.py` | DOM 유사도 기반 로컬 자가치유 (비용 0) |
| `shared/zero_touch_qa/executor.py` | DSL 실행 오케스트레이터 + 3단계 하이브리드 Self-Healing |
| `shared/zero_touch_qa/converter.py` | Playwright codegen 녹화 스크립트 → 9대 DSL 변환 |
| `shared/zero_touch_qa/report.py` | HTML 리포트 + run_log.jsonl 생성 |
| `shared/zero_touch_qa/regression_generator.py` | 성공 시나리오 → 독립 Playwright 회귀 테스트 자동 생성 |
| `shared/zero_touch_qa/utils.py` | JSON 추출, 이미지 압축 등 공용 유틸리티 |
...
**14대 표준 DSL 준수 (v4.1 확장)**

AI의 환각(Hallucination)을 원천 차단하면서도 복잡한 웹 UI 및 네트워크 제어를 위해 두 계층은 다음 14개의 표준 액션으로만 소통한다.
...
| `verify` | 요소 상태/속성/텍스트 확인 | `{"action": "verify", "target": "#result", "condition": "visible"}` |
| `upload` | `<input type="file">` 에 파일 업로드 | `{"action": "upload", "target": "#file-upload", "value": "test.png"}` |
| `drag` | 요소를 끌어서 다른 요소에 드롭 | `{"action": "drag", "target": "#item-1", "value": "#drop-zone"}` |
| `scroll` | 요소가 보일 때까지 스크롤 | `{"action": "scroll", "target": "#footer", "value": "into_view"}` |
| `mock_status` | API 응답 상태 코드 가로채기(조작) | `{"action": "mock_status", "target": "**/api/users", "value": "500"}` |
| `mock_data` | API 응답 JSON 바디 조작 | `{"action": "mock_data", "target": "**/api/data", "value": "{\"items\":[]}"}` |

`verify`의 `condition` 은 v4.1 기준으로 `visible`, `hidden`, `disabled`, `enabled`, `checked`, `value` 를 허용한다. `condition` 이 없고 `value` 만 있으면 텍스트 포함 검증으로 해석한다.
...
| v4.0 | **전면 재작성.** 980줄 단일 파일(`mac_local_executor.py`)을 11개 모듈 패키지(`shared/zero_touch_qa/`)로 분리. Config dataclass로 설정 일괄 관리. DifyClient의 `generate_scenario()`/`request_healing()` 용도별 메서드 분리. 치유 요청 시 `failed_step` 컨텍스트 추가. Self-Healing 3단계 체계 재정립(fallback_targets → 로컬 유사도 → Dify LLM). CLI 모드가 사용자 Flow와 1:1 대응(`--mode chat\|doc\|convert\|execute`). convert 모드가 변환+실행을 단일 호출로 체이닝. Dify 연결 실패 시 에러 리포트 HTML 자동 생성. Jenkinsfile v4.0 전면 개편(`python3 -m zero_touch_qa` 방식). |
| v4.1 | **Action DSL 확장.** 기존 9대 액션에서 14대 액션으로 통제된 확장 수행. `upload`, `drag`, `scroll`을 통한 고급 UI 상호작용 지원 및 `mock_status`, `mock_data`를 통한 네트워크 엣지 케이스(Error, Empty state) 제어 기능 추가. `verify` 액션에 `condition` 속성을 도입해 상태 검증 고도화. |
...
```
기획서(PDF/Word) → [Dify Parser] → TC 추출 → [Planner LLM] → 14대 DSL JSON
...
```
자연어("로그인해줘") → [Planner LLM] → 14대 DSL JSON
    → [Python Executor] → LocatorResolver → 브라우저 실행 → 산출물
...
사용자는 코드를 작성할 필요 없이, 목적에 맞는 진입 경로를 선택하면 모든 결과가 표준 DSL로 수렴하여 실행된다. 다만 v4.1 기준에서 14대 DSL은 Chat/Doc 경로의 목표 계약이며, Convert 경로는 현재 9대 DSL 변환기에서 시작해 Sprint 2 이후 확장될 예정이다.

##### 5.8.3.1 Flow 1: 문서 업로드 (Doc-to-Test)

- **목적:** 기획서 기반 대량 시나리오 구축
- **사용자 행동:** Dify 화면에 요구사항 정의서(PDF/Word)를 업로드한다.
- **시스템 흐름:** Dify 파서가 문서를 읽고 테스트 케이스(TC)를 분리한다. Planner LLM이 각 TC를 14대 DSL로 번역한다. 파이썬 엔진이 실행한다.

##### 5.8.3.2 Flow 2: 대화형 직접 입력 (Chat-to-Test)

- **목적:** 신규 기능의 즉각적인 단건 검증 및 디버깅
- **사용자 행동:** Dify 채팅창에 자연어로 입력한다. (예: "네이버 검색창에 DSCORE 치고 엔터 눌러줘")
- **시스템 흐름:** Planner LLM이 즉시 의도를 파악해 14대 DSL로 번역한다. 대상이 모호할 경우 Dify가 채팅으로 사용자에게 되묻는다(HITL). 파이썬 엔진이 실행한다.
...
- **시스템 흐름:** 녹화된 `.py` 스크립트를 `--mode convert`로 변환하면, 정규식 파서가 Playwright API 호출(`get_by_role`, `fill`, `click` 등)을 현재 9대 DSL로 1:1 매핑하여 `scenario.json`을 생성한다. LLM 호출 없이 로컬에서 즉시 변환된다. Convert 경로의 14대 DSL 확장은 별도 Sprint 2 이후 작업이다. 생성된 시나리오는 Jenkins에 업로드하거나 `--scenario` 옵션으로 즉시 실행할 수 있다.
...
| 5 | `dom` | **Paragraph** | 아니오 | Heal 모드 전용. HTML DOM 스냅샷 (권장 4,000자 이하, 운영 상한 6,000자) |
| 6 | `failed_step` | **Paragraph** | 아니오 | Heal 모드 전용. 실패한 스텝의 JSON 문자열 (액션, 원래 타겟 등 포함) |
| 7 | `api_docs` | **Paragraph** | 아니오 | Chat/Doc 모드용. 대상 애플리케이션의 주요 API 엔드포인트 명세 (네트워크 모킹 환각 방지용) |
...
Chat/Doc 모드에서 자연어 요구사항을 14대 DSL JSON으로 변환하는 핵심 노드이다. 다만 Flow 3(convert)는 여전히 로컬 정규식 파서의 9대 DSL 변환 경로를 사용한다.
...
우측 패널 상단에서 모델을 선택한다:
- **기본 권장:** `gemma4:e4b`
- **허용 대안:** `llama3.1:8b`, `qwen2.5:7b` 등 7B급 로컬 모델
- **Sprint 1 비기본:** 30B급 로컬 모델 또는 외부 SaaS 모델은 문서 기본선에서 제외한다

**Completion Params 권장값:**
...
- step 1 은 항상 navigate 입니다:
  { "step": 1, "action": "navigate", "target": "", "value": "<target_url>", "description": "대상 페이지 로드" }
- target_url 값을 step 1 value 에 그대로 넣습니다.
- JSON 배열([...])만 출력합니다. 마크다운, 부연 설명, `<think>` 류 내부 추론 텍스트는 금지합니다.
...
- verify: target="검증 대상 요소". 
  > 텍스트 검증 시: value 사용
  > 상태 검증 시: condition="visible", "hidden", "disabled", "enabled", "checked", "value"
...
[작성 규칙]
- 시맨틱 로케이터(`role+name`, `text=`, `label=`, `placeholder=`)를 우선 사용합니다.
- name 없는 단독 role (`role=button`, `role=link`) 은 금지합니다.
- 복잡한 CSS/XPath 추측은 금지합니다.
- `mock_status`, `mock_data` 는 API 호출 유발 액션보다 먼저 배치합니다.
- `wait` 는 최소화하고 상태 확인은 `verify` 로 표현합니다.
- `press` 의 키 이름은 target 이 아니라 value 에 넣습니다.
- `verify` 에서 value 가 비어 있으면 condition 을 함께 넣습니다.
- `api_docs`가 제공되면 네트워크 모킹 대상 URL 패턴을 정할 때 참고하되, 장문 설명을 그대로 복사하지 말고 필요한 엔드포인트만 반영합니다.
- "첫 번째 검색결과 링크 클릭" 같은 동적 항목 클릭은
  `target: "main a[href]:visible"` 와
  `fallback_targets: ["article a[href]:visible", "[id*='result' i] a[href]:visible", "[class*='result' i] a[href]:visible"]`
  를 우선 사용합니다.
...
출력:
[
  {"step": 1, "action": "navigate", "target": "", "value": "https://example.com/profile", "description": "대상 페이지 로드", "fallback_targets": []},
  {"step": 2, "action": "mock_status", "target": "**/api/profile", "value": "500", "description": "저장 API 500 에러 모킹", "fallback_targets": []},
  {"step": 3, "action": "click", "target": "role=button, name=저장", "value": "", "description": "저장 버튼 클릭", "fallback_targets": ["text=저장", "[type='submit']"]},
  {"step": 4, "action": "verify", "target": "text=저장에 실패했습니다", "value": "", "condition": "visible", "description": "에러 팝업 노출 검증", "fallback_targets": ["role=alert", "text=실패"]}
]
```
...
> Sprint 1 기준 결정: Dify Chatflow 내부에 별도 JSON parser / guard 노드는 추가하지 않는다. Chatflow는 짧은 계약과 출력 제한에 집중하고, 구조 검증/재시도/보정은 Python 후단(`zero_touch_qa`)이 담당한다.
...
- **기본 권장:** `gemma4:e4b`
- **허용 대안:** 7B급 로컬 모델
- Heal 품질이 더 필요하면 모델을 키울 수는 있지만, Sprint 1의 문서 기준선은 16GB + `gemma4:e4b` 이다.
...
- DOM 스냅샷은 일부만 전달될 수 있으므로, 긴 XPath 대신 짧고 안정적인 시맨틱 셀렉터를 우선하십시오.

[신규 액션별 규칙]:
- drag: 목적지 문제면 새 목적지 로케이터를 "value" 에 넣습니다.
- upload: 숨겨진 `<input type="file">` 를 우선 찾습니다.
- mock_status / mock_data: DOM 셀렉터가 아니라 target URL 패턴을 교정합니다.
...
- 부연 설명은 절대 금지합니다.
- `<think>` 같은 내부 추론 텍스트는 절대 출력하지 않습니다.
```
...
#### 5.8.7 Jenkins Pipeline (`ZeroTouch-QA`) 전체 스크립트
...
**파일 경로:** `data/jenkins/scripts/ZeroTouch-QA.jenkinsPipeline`
...
| 6 | Pipeline Job 생성 | Jenkins에 `ZeroTouch-QA` Pipeline Job을 생성하고 Script에 Jenkinsfile 붙여넣기 | 바로 아래 참고 |

**Jenkins Pipeline Job 생성 절차:**

1. Jenkins 메인 > `새로운 Item` > 이름: `ZeroTouch-QA` > `Pipeline` 선택 > `OK`
2. Pipeline 섹션에서 Definition: `Pipeline script` 선택
3. Script 입력란에 `ZeroTouch-QA.jenkinsPipeline` 파일 내용을 그대로 붙여넣기
4. `저장` 클릭
...
1. Jenkins에서 `ZeroTouch-QA` Job을 연다.
...
- 여러 단계가 있으면 순서대로 나열한다. Chat/Doc 경로에서는 AI가 14대 DSL로 변환한다.
...
1. Jenkins에서 `ZeroTouch-QA` Job > `Build with Parameters`를 연다.
2. 파라미터를 입력한다:
...
| 항목 | 설명 | 상태 |
| --- | --- | --- |
| 전면 모듈화 | 980줄 단일 파일을 11개 모듈 패키지(`shared/zero_touch_qa/`)로 분리 | v4.0 완료 |
