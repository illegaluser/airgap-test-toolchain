...
[✅ 복합 시나리오 예시 (파일 업로드 및 드래그 앤 드롭)]
입력: "프로필 사진을 업로드하고, 완료되면 카드를 드래그해서 휴지통에 넣어"
출력:
[
  {"step": 1, "action": "upload", "target": "input[type='file']", "value": "fixtures/profile.png", "description": "프로필 사진 파일 업로드", "fallback_targets": [".file-upload-input"]},
  {"step": 2, "action": "drag", "target": "text=내 프로필 카드", "value": "role=region, name=휴지통", "description": "카드를 휴지통 영역으로 드래그", "fallback_targets": [".profile-card", "#trash-bin"]}
]
