...
[Knowledge Retrieval #1: kb_project_info]
  ├── dataset_id = __KB_PROJECT_INFO_ID__   ← provision.sh 가 substitute
  ├── query = user_query + (target_module 있으면 concat)
  ├── top_k = 3, score_threshold = 0.5      ← 보수적 시작 (KB context 부담 ↓)
  ▼
[Knowledge Retrieval #2: kb_test_theory]
  ├── dataset_id = __KB_TEST_THEORY_ID__    ← provision.sh 가 substitute
  ├── query = user_query
  ├── top_k = 2, score_threshold = 0.6      ← 보수적 시작
  ▼
[Variable Aggregator]
...
**spike 필요 사항** (T-spike-2 로 별도 태스크화):

- Dify 1.13.3 의 chatflow YAML 안 Knowledge Retrieval 노드 schema 가 정확히 어떤 필드명
  으로 dataset 을 참조하는지 (`dataset_ids`? `data.knowledge_id`?). 문서가 아닌 **실제
  console 에서 만든 chatflow 를 export 해서** 확인.
- ID substitute 시 YAML 의 다른 필드와 충돌이 없는지 (placeholder 가 16진수 hash 처럼
  보이는 다른 필드명에 우연히 매칭되지 않게 prefix `__KB_` 형태로 잡아 unique 보장).
- 두 KB 가 모두 만들어지기 전에 chatflow import 를 시도하면 어떤 에러가 나는지
  (validation 시점). spike 결과로 idempotent 처리 정책 결정.
...
**context budget 안전 확인** (Sprint 6 OLLAMA_CONTEXT_SIZE=12288 기준):

- system prompt: ~3000 tokens (mode 분기 + 1-shot 셋)
- user query + target_module: ~200 tokens
- retrieved context: 5 chunks × ~250 tokens = ~1250 tokens
- LLM output (`max_tokens=8192`): 8192 tokens
- **합계**: ~12642 tokens — context 12288 살짝 초과 가능

→ 따라서 top_k 보수적 시작 (3+2=5 chunks) 으로 ~1000 tokens 차감, 안전 마진 확보.
T-15 (retrieval recall 평가) 후 **품질 부족 시 5+3 로 상향**. 충돌 시 OLLAMA_CONTEXT_SIZE
를 16384 로 상향 (Sprint 6 §11.3.3 보다 본 트랙은 retrieval context 가 동적 길이라 더
큰 마진이 필요함). 이 결정은 §9 결정 사항에 기록.

이 값은 **초기 default**. T-12 검증 후 retrieval 품질이 낮으면 chunk_size 300 또는 700
로 조정 검토.

### 4.4 KB 사용 가이드라인 (저작권 / 민감 정보)
...
### 5.0 Phase -1 — Spike (구현 위험 사전 해소)

설계 가정 검증을 코드 작성 전에 수행. 결과를 본 PLAN §2.2.3 / §5.x 에 반영 후 구현 진입.

| ID | 작업 | 산출물 | 검증 |
| --- | --- | --- | --- |
| T-spike-1 | Dify 1.13.3 의 KB 생성 API body schema 실측 — Dify console GUI 에서 manual 로 KB 한 개 생성 후 `/console/api/datasets` GET 으로 응답 구조 캡처 + 동일 body 로 POST 가능한지 curl 검증 | `examples/spike/kb-create-fixture.json` (실측 body) | curl 로 KB 1개 생성 + 즉시 삭제 → 200/201 |
| T-spike-2 | Chatflow YAML 의 Knowledge Retrieval 노드 schema 확인 — Dify console 에서 KB 1 개 + chatflow 1 개 (Retrieval 노드 포함) 생성 → DSL export → YAML 안 dataset 참조 필드명 / 형식 (UUID? 이름?) 정확히 식별 | `examples/spike/retrieval-node-fixture.yaml` | export 결과에서 dataset_id 필드 위치 명시 + 본 PLAN §2.2.3 갱신 |
| T-spike-3 | Dataset id substitute 메커니즘 PoC — placeholder (`__KB_PROJECT_INFO_ID__`) 가 박힌 mock chatflow YAML 을 만들고, sed 또는 python str.replace 로 실 ID 주입 후 import → 정상 동작 확인 | spike 스크립트 | mock import 로 chatflow 가 두 KB 모두 retrieve 하는지 검증 |
