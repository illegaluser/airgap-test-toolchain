...
| 2026-04-27 | 검증 결과 | `pytest test/test_sprint2_runtime.py` 16/16 PASS — Sprint 2 DoD 8 항목 모두 충족. Sprint 2 "구현 완료" 선언 (§6 헤더) | §6.4.2 |
...
```text
$ pytest test/test_sprint2_runtime.py -v
============================== 16 passed in 0.10s ==============================
```
...
Sprint 3 은 **Sprint 2 에서 구현한 14대 DSL 런타임이 진짜로 동작하는지 fixture 기반 pytest 로 잠그는 검증 스프린트**다. 범위는 `playwright-allinone/test/` 의 fixture HTML 과 pytest 케이스, 그리고 Jenkins 파이프라인의 pytest 단계 추가까지다. Dify Brain 의 LLM 응답 품질 검증은 Sprint 1 책임이며, 실제 운영 환경 통합은 Sprint 4 로 분리한다.
...
| ID | 작업 | 산출물 | 완료 기준 | 상태 |
| --- | --- | --- | --- | --- |
| S3-01 | upload fixture + pytest | `test/fixtures/upload.html`, `test/test_upload.py` | `<input type="file">` 에 artifacts 안 파일 업로드 성공 / artifacts 밖 경로 차단 / 빈 value 거부 — 최소 3 케이스 | 미착수 |
| S3-02 | drag fixture + pytest | `test/fixtures/drag.html`, `test/test_drag.py` | source→destination drop 후 DOM 변화 검증 / 목적지 미존재 시 RuntimeError 명시 — 최소 2 케이스 | 미착수 |
| S3-03 | scroll fixture + pytest | `test/fixtures/scroll.html`, `test/test_scroll.py` | viewport 밖 요소가 scroll 후 visible / `into_view` 외 value 는 `_validate_scenario` 단계에서 거부 — 최소 2 케이스 | 미착수 |
| S3-04 | mock_status + UI 예외처리 시나리오 | `test/fixtures/mock_status.html`, `test/test_mock_status.py` | 500 모킹 → fetch 호출 → DOM 에 에러 UI 노출 → `verify` 로 검증 / `times=1` 기본 / `times=3` 다중 가로채기 — 최소 3 케이스 | 미착수 |
| S3-05 | mock_data + 빈 응답 시나리오 | `test/fixtures/mock_data.html`, `test/test_mock_data.py` | dict body / JSON 문자열 / 평문 모두 정상 fulfill / 빈 배열 응답 후 "데이터 없음" UI 노출 검증 — 최소 3 케이스 | 미착수 |
| S3-06 | verify.condition 분기별 pytest | `test/fixtures/verify_conditions.html`, `test/test_verify_conditions.py` | hidden/disabled/enabled/checked/value/text 6 분기 + 빈 condition + 알 수 없는 condition 거부 — 최소 7 케이스 | 미착수 |
| S3-07 | regression_generator 산출물 실행 검증 | `test/test_regression_emit_runs.py` | 14대 액션 시나리오로 `generate_regression_test` 호출 → 생성된 `regression_test.py` 를 subprocess 로 실행해 종료코드 0 — 최소 1 케이스 | 미착수 |
| S3-08 | 14대 액션 통합 시나리오 | `test/test_executor_full_dsl.py` | 14대 액션을 한 시나리오에 모두 포함해 fixture HTML 위에서 순차 실행, run_log.jsonl 에 14건 PASS / scenario.healed.json 정합 — 최소 1 케이스 | 미착수 |
| S3-09 | fallback heal 통합 검증 | `test/test_healing_fallback.py` | 1차 target 실패 → `fallback_targets` 로 복구 → `scenario.healed.json` 에 갱신된 target 직렬화 확인 — 최소 1 케이스 | 미착수 |
| S3-10 | mock_* 치유 경로 통합 검증 | `test/test_mock_healing.py` | 잘못된 URL 패턴 → fallback URL 복구 / Dify LLM heal 은 monkeypatch 로 결정론적 응답 — 최소 2 케이스 | 미착수 |
| S3-11 | 3-Flow 통합 회귀 | `test/test_three_flows.py` | chat (Dify monkeypatch) / doc (로컬 step parser) / execute (손작성 scenario.json) 3 모드 모두 fixture 위에서 통과 — 최소 3 케이스 | 미착수 |
| S3-12 | airgap 가드 | `test/test_airgap_guard.py` 또는 conftest 에 파일 스캔 | `test/fixtures/` 안 모든 HTML 이 `http://` / `https://` 로 시작하는 URL 을 참조하지 않음을 자동 검증 — 최소 1 케이스 | 미착수 |
| S3-13 | Jenkins 파이프라인 pytest 단계 추가 | `DSCORE-ZeroTouch-QA-Docker.jenkinsPipeline` | `pytest test/ --junitxml=...` 단계가 본 파이프라인의 pre-deploy 위치에 존재, 실패 시 빌드 fail, JUnit XML archiveArtifacts | 미착수 |
| S3-14 | Sprint 3 결과 요약 | 본 계획서 §7 (현재 섹션) | 단위테스트 수, 신규 fixture 목록, 회귀 차단 범위가 본 계획서에 갱신되어 Sprint 4 로 인계 가능 | 미착수 |
...
**Gate**: Phase 0 산출물만으로 빈 시나리오 실행이 동작해야 다음 단계 진입. `pytest test/test_sprint2_runtime.py` 16 건 회귀 무결.
...
2. **S3-13 Jenkins** — `DSCORE-ZeroTouch-QA-Docker.jenkinsPipeline` 의 build 단계 직전에 `pytest test/ --junitxml=artifacts/pytest.xml` stage 추가. JUnit XML archiveArtifacts. 실패 시 `unstable` 이 아니라 `fail`.
...
### 7.6 Sprint 3 종료 조건 (Definition of Done)

- 14대 액션 모두 1건 이상의 fixture 기반 pytest 케이스를 가진다.
- Sprint 2 단위테스트(16건) 와 Sprint 3 신규 케이스가 모두 PASS — `pytest test/` 가 회귀 없이 통과.
- 모든 fixture 가 `file://` 만 참조하고 외부 도메인 0 — S3-12 가드가 자동 보장.
- `regression_test.py` 산출물이 별도 프로세스에서 실행되어 종료코드 0 (S3-07).
- Healing 루프가 fallback / mock fallback 경로에서 `scenario.healed.json` 에 갱신된 target 을 직렬화함이 검증된다 (S3-09, S3-10).
- 3-Flow 통합 시나리오가 chat/doc/execute 모두 PASS (S3-11).
- Jenkins 파이프라인이 pytest 단계를 포함하고 빌드 실패 시 stop (S3-13).
- 본 계획서 §7.4 표가 모두 "완료" 로 갱신되어 Sprint 4 가 E2E 통합에 착수 가능하다.

### 7.7 Sprint 3 비범위 (Out of Scope)
...
- **Sprint 3 (진행 예정)**: §7 의 14건 작업으로 14대 DSL 의 fixture 기반 pytest 회귀, mock_* 의 UI 예외처리 시나리오, 3-Flow 통합, regression_test.py 산출물 실행 검증, Jenkins pytest 단계 추가까지 잠근다.
