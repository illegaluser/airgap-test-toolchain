...
Sprint 2는 **문서 계약을 실제 Python 실행기로 연결하는 구현 스프린트**다. 범위는 `zero_touch_qa` 런타임과 Jenkins 파이프라인 입력까지이며, Dify 운영/배포 계층 안정화는 포함하지 않는다.
...
| ID | 작업 | 산출물 | 완료 기준 | 상태 |
| --- | --- | --- | --- | --- |
| S2-01 | `_VALID_ACTIONS`를 14대 DSL로 확장 | `zero_touch_qa/__main__.py` | `upload`/`drag`/`scroll`/`mock_status`/`mock_data`가 구조 검증에서 거부되지 않음 | 미착수 |
| S2-02 | 액션별 필수 필드 검증 추가 | `zero_touch_qa/__main__.py` | `drag.value`, `upload.value`, `scroll.value=into_view`, `mock_status.value` 숫자성 등 최소 계약을 검증 | 미착수 |
| S2-03 | `_perform_action()`에 신규 5종 액션 구현 | `zero_touch_qa/executor.py` | 신규 액션이 실제 Playwright API 호출로 실행됨 | 미착수 |
| S2-04 | upload 경로 해석/방어 로직 추가 | `zero_touch_qa/executor.py` | 허용 루트 밖 경로(`../`, 절대경로 탈출`)가 차단되고 정상 파일만 업로드 가능 | 미착수 |
| S2-05 | drag 목적지 사전 검증과 예외 메시지 정리 | `zero_touch_qa/executor.py` | source/target 중 어느 쪽 실패인지 로그와 예외에 남음 | 미착수 |
| S2-06 | scroll 계약 고정 | `zero_touch_qa/__main__.py`, `executor.py` | `scroll.value`가 `into_view` 외 값이면 조기 실패 또는 normalize | 미착수 |
| S2-07 | mock route 설치/수명주기 관리 | `zero_touch_qa/executor.py` | `mock_*`가 정확히 필요한 시점에만 적용되고 후속 스텝을 오염시키지 않음 | 미착수 |
| S2-08 | `verify.condition` 분기 구현 | `zero_touch_qa/executor.py` | `visible`/`hidden`/`disabled`/`enabled`/`checked`/`value`가 각각 적절한 `expect()` 호출로 매핑 | 미착수 |
| S2-09 | verify 실패 원인 분리 | `zero_touch_qa/executor.py` | locator 탐색 실패와 assertion 실패가 구분된 에러로 남아 Healer 오동작을 줄임 | 미착수 |
| S2-10 | Healer 병합 규칙 점검 | `zero_touch_qa/executor.py` | `drag`/`mock_*`에서 healer가 `value`를 바꿔도 런타임이 이를 반영 | 미착수 |
| S2-11 | `execute` 모드 호환성 확보 | `zero_touch_qa/__main__.py`, `executor.py` | 외부 scenario.json에 신규 액션이 포함돼도 동일하게 실행 가능 | 미착수 |
| S2-12 | `scenario.healed.json` 반영 범위 점검 | `zero_touch_qa`, 리포트 산출물 | 신규 액션 치유 결과(`target`, 필요 시 `value`)가 최종 시나리오에 저장됨 | 미착수 |
| S2-13 | Jenkins 입력 계약 정리 | `DSCORE-ZeroTouch-QA-Docker.jenkinsPipeline`, README | `API_DOCS` 등 Sprint 1에서 추가한 입력이 Sprint 2 런타임과 일관되게 유지됨 | 미착수 |
| S2-14 | 최소 smoke 검증 절차 문서화 | 본 계획서 또는 README | pytest 전에도 신규 액션별 수동/간이 점검 순서 존재 | 미착수 |
...
### 6.6 Sprint 2 종료 조건 (Definition of Done)

- `chat`, `doc`, `execute`에서 신규 5종 액션을 포함한 시나리오가 구조 검증을 통과한다.
- `executor.py`가 신규 5종 액션을 런타임 예외 없이 수행할 수 있다.
- `verify.condition` 상태 검증이 구현되고, assertion 실패와 locator 실패가 구분된다.
- `upload` 경로 방어와 `mock_*` 수명주기 제한이 코드에 존재한다.
- Healer가 바꾼 `target`/`value`가 신규 액션에도 일관되게 반영된다.
- Sprint 3 테스트팀이 바로 fixture/pytest 작성에 착수할 수 있을 만큼 입력 계약과 로그/산출물 경계가 고정된다.
