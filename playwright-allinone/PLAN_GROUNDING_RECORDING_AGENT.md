...
| 공유 경로 | `~/.dscore.ttc.playwright-agent/recordings/<id>/` (호스트) ↔ `/data/recordings/<id>/` (컨테이너) |
...
사용자 → Recording Web UI (http://localhost:18092)
       ...
       Recording Service
          ...
          └─ docker exec <container> python -m zero_touch_qa \
                ...
                --file /data/recordings/<id>/original.py
                                               ─────────────────→
                                          [컨테이너 측 처리]
                                          ...
                                          _validate_scenario()
                                              ↓
                                          /data/recordings/<id>/scenario.json 저장
                                              ↓
...
- 호스트: `original.py` 를 공유 볼륨 (`/data/recordings/<id>/`) 에 저장 + `docker exec` 호출
...
| `provision.sh` | — | — | — | — | (모델 교체 시) plugin-model 등록 7단계 | — |
