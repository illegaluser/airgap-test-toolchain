...
- `recording-ui/recording_service/web/` — HTML / CSS / JS (메인)
- `recording-ui/recording_service/server.py` — endpoint 신설
- `recording-ui/recording_service/rplus/router.py` — 비교 endpoint
...
2. **🔍 Discover URLs** — 토글 *(default 닫힘)* — seed_url, auth profile, max
   ...
   *(R8)* 결과 표 아래 **사이트 계층 트리** 카드 — 크롤 토폴로지 / URL 경로
   두 탭 + 단일 HTML 다운로드.
   *(R9)* 결과 표 위 **사이트 스캔 통계** 카드 — cap 도달 경고 + sitemap
   커버리지 + depth/source/status 분포.
...
GET  /discover/{job_id}/tree?type=crawl|path        (R8 — 사이트 계층 트리 JSON)
GET  /discover/{job_id}/tree.html                   (R8 — 두 트리 단일 HTML)
...
| 변경 | 위치 |
| --- | --- |
| `analyze_codegen_vs_regression()` + `DiffAnalysisResult` | `recording-ui/recording_service/enricher.py` |
| `POST /experimental/sessions/{sid}/diff-analysis` | `recording-ui/recording_service/rplus/router.py` |
...
신규: [recording-ui/recording_service/codegen_trace_wrapper.py](../recording_service/codegen_trace_wrapper.py).
...
[recording-ui/recording_service/replay_proxy.py:199-260](../recording_service/replay_proxy.py#L199-L260) 의 `run_codegen_replay()` 변경:
...
신규: [recording-ui/recording_service/trace_parser.py](../recording_service/trace_parser.py).
...
[recording-ui/recording_service/server.py](../recording_service/server.py):
...
[recording-ui/recording_service/web/index.html](../recording_service/web/index.html):

- `#run-log-card` 헤더에 `<div class="run-log-mode-tabs">` —
  `[LLM] [원본]` 토글 버튼.

[recording-ui/recording_service/web/app.js](../recording_service/web/app.js):
...
[recording-ui/recording_service/web/style.css](../recording_service/web/style.css):
...
- `playwright-allinone/shared/recording_shared/codegen_trace_wrapper.py`
- `playwright-allinone/shared/recording_shared/trace_parser.py`
- `playwright-allinone/test/test_codegen_trace_wrapper.py`
- `playwright-allinone/test/test_trace_parser.py`

**수정**:

- `playwright-allinone/recording-ui/recording_service/replay_proxy.py` — wrapper 호출 +
  trace 파싱 후처리
- `playwright-allinone/recording-ui/recording_service/server.py` — `/run-log`,
  `/screenshot/{name}` 에 `mode` 쿼리, 응답 wrap, jpeg 화이트리스트
- `playwright-allinone/recording-ui/recording_service/web/index.html` — 모드 탭 마크업
- `playwright-allinone/recording-ui/recording_service/web/app.js` — `_runLogState`,
  `_renderRunLog` mode-aware 재구현, 탭 핸들러, 스크린샷 모달 mode 전달
- `playwright-allinone/recording-ui/recording_service/web/style.css` — `.run-log-mode-tabs`
...
| 파일 | 변경 |
| --- | --- |
| `recording-ui/recording_service/web/app.js` | `_prettifyErrorMessage` (Discover 에러 포매터), `_refreshPreviewToggle` (토글 가시성 결정), `_renderHealedScenario` / `_renderPlayLlmLog` 추가, `openSession` 에서 호출 |
| `recording-ui/recording_service/web/index.html` | `#discover-status` `white-space: pre-wrap`, 신규 카드 2개 + 모든 미리보기 박스에 `class="collapsible"` + `.preview-toggle` 버튼 |
| `recording-ui/recording_service/web/style.css` | `.json-preview.collapsible.expanded`, `.preview-toggle` |
| `recording-ui/recording_service/storage.py` | `scenario_healed_path`, `play_llm_log_path` 헬퍼 |
| `recording-ui/recording_service/server.py` | 신규 엔드포인트 2개 |
...
| 파일 | 변경 |
| --- | --- |
| `recording-ui/recording_service/web/index.html` | 5개 → 4개 토글 카드 재편, sub-block 도입, "Play with LLM" → "LLM 적용 코드 실행" |
| `recording-ui/recording_service/web/app.js` | `_renderHealedScenario` / `_renderRegression` 을 sub-block 대상으로 조정, `_runPlay` 완료 시 4개 카드 + diff 자동 재렌더, 카드 details 자동 open/close 로직 |
| `recording-ui/recording_service/web/style.css` | `.card-toggle > details > summary`, `.card-actions`, `.sub-block` |
...
- 신규 모듈 `recording-ui/recording_service/report_export.py`
...
| 파일 | 변경 |
| --- | --- |
| `recording-ui/recording_service/report_export.py` | (신규) self-contained HTML 생성기 |
| `recording-ui/recording_service/server.py` | `/report` endpoint 추가 |
| `recording-ui/recording_service/web/index.html` | 결과 카드 헤더에 `📤 리포트 다운로드` 추가 |
| `recording-ui/recording_service/web/app.js` | `_renderRunLog` 가 `#dl-report` href wire |
| `shared/zero_touch_qa/executor.py` | `StepResult.dialog_text`, `page.on("dialog")` 훅 |
| `shared/zero_touch_qa/report.py` | `save_run_log` 가 `dialog_text` 있을 때 jsonl 에 포함 |
...
## Round 8 — Discover 결과 사이트 계층 트리 출력 (2026-05-02)

### 배경 (R8)

운영자가 Discover URLs 결과 (수십~수백 URL flat list) 를 보고 "이 사이트는
어떻게 구성돼 있는지" 를 한눈에 파악하기 어려웠다. 외부 검수자에게 IA 검토
요청 시에도 평면 표를 그대로 보내야 해서 의미 파악에 시간 소모.

분석 결과 — Discovery 의 BFS 가 큐에서 URL 을 꺼낼 때 "이 URL 을 어느 페이지에서
발견했는지" (parent / referrer) 를 잃어버리고 있었음. 사후 트리 재구성에 필요한
정보가 아예 없는 상태였음.

### 설계 결정 (R8)

- **두 가지 트리를 동시 제공** — 사용자가 비교 가능하도록.
  - **크롤 토폴로지** — `parent_url` 기반. 사이트의 *실제 링크 그래프* 그대로.
  - **URL 경로 기반** — path segment 누적. 사이트의 *URL IA* 그대로. SPA / 쿼리
    식별자에는 약하지만 정적 사이트엔 정확.
- **BFS 에 parent_url 1 필드 추가** — 큐 튜플을 4-tuple 로 확장 (`(url, depth,
  source, parent_url)`). seed 와 sitemap 출처는 None, anchor / request /
  spa_selector 는 발견된 페이지의 *최종* URL (리다이렉트 후).
- **결과 패널 안 트리 뷰 + self-contained HTML 다운로드** — R7 의 결과 리포트와
  같은 "단일 HTML, 외부 의존성 0" 패턴 차용.

### 변경 묶음 (R8)

#### 1. parent_url 추적 (BFS)

- `DiscoveredUrl.parent_url: Optional[str]` 필드 추가.
- BFS 큐 튜플 `(url, depth, source)` → `(url, depth, source, parent_url)`.
- 자식 push 시점에 현재 처리 중인 페이지의 `final_url` (리다이렉트 후) 을 parent
  로 저장 — 운영자가 실제 본 페이지를 부모로 인식.
- CSV fieldnames 끝에 `parent_url` 컬럼 append (기존 인덱스 보존).

#### 2. tree_builder 모듈 (신규)

- `recording-ui/recording_service/tree_builder.py`
  - `build_crawl_tree(records, seed_url) -> {root, orphans}`
  - `build_path_tree(records, seed_url) -> {root, external}`
  - `render_self_contained_tree_html(crawl, path, meta) -> str`
- 두 트리 동일 노드 스키마 — `{ url, title, depth, source, status, children }`.
- HTML 은 인라인 CSS + 탭 전환 JS 만 포함, 외부 자산 0.

#### 3. 신규 endpoint

- `GET /discover/{job_id}/tree?type=crawl|path` — JSON
- `GET /discover/{job_id}/tree.html` — self-contained HTML 다운로드

#### 4. UI

- `#discover-result` 아래 `<details id="discover-tree-toggle">` 카드 추가.
- 두 탭 (`크롤 토폴로지` / `URL 경로`), 활성 탭 pane 만 표시.
- 트리 노드는 중첩 `<ul>` + `<details>` 토글, 깊이 ≤ 1 만 기본 펼침.
- 헤더에 `📤 트리 다운로드 (HTML)` 링크.
...
| 파일 | 변경 |
| --- | --- |
| `shared/zero_touch_qa/url_discovery.py` | `DiscoveredUrl.parent_url`, BFS 큐 4-tuple 화, push/pop 갱신 |
| `recording-ui/recording_service/server.py` | `/discover/{id}/tree` + `/tree.html` 2 endpoint, CSV fieldnames |
| `recording-ui/recording_service/tree_builder.py` | (신규) 두 트리 빌더 + HTML 렌더 |
| `recording-ui/recording_service/web/index.html` | `#discover-tree-toggle` 카드 + 탭 + 다운로드 링크 |
| `recording-ui/recording_service/web/app.js` | `_renderDiscoverTrees(jobId)` + 노드 렌더 + 탭 전환 핸들러 |
| `recording-ui/recording_service/web/style.css` | `.tree-tabs`, `ul.tree`, `.tree-badge`, `.tree-pane` |

### 회귀 (R8)

- `node --check` (app.js) / `ast.parse` (server.py / url_discovery.py / tree_builder.py) 통과.
- url_discovery 단위 24개 격리 실행 통과.
- discover e2e (`test_discover_submits_and_renders_result_table`) 격리 실행 통과.
- 기존 `urls.json` 산출물(parent_url 없음)도 endpoint 가 정상 처리 — 모든
  비 seed 가 orphans 그룹으로 떨어짐.

### 위험과 회피 (R8 추가분)

| 위험 | 회피 |
| --- | --- |
| `parent_url` 추가가 외부 urls.json/CSV 컨슈머 깸 | JSON 은 새 키 추가만, CSV 는 fieldnames 끝에 append. `.get()` 만 쓰면 backward compat |
| 큰 사이트 트리의 DOM 비대 | 깊이 ≥ 2 부터 기본 접힘 + summary 에 자손 카운트 표시 — 한 번에 수백 노드 펼침 회피 |
| path tree 가 SPA / `?id=` 식별자에 부정확 | 두 트리 동시 제공 — 운영자가 비교 가능. UI 에 한계 안내 명시 |
| 트리 큐 튜플 형식 변경이 progress callback 영향 | `current_url` 을 push 시점에 캡처해 튜플 끝 위치에 추가 — 위치 인덱스만 변경, 분기/흐름 동일 |

---

## Round 9 — Discover 사이트 스캔 통계 노출 (2026-05-02)

### 배경 (R9)

R8 후에도 운영자가 결과 패널 상태 라인 (`[done] N건`) 만 보고 "이게 사이트 전체인지,
`max_pages` 200 캡에 닿아 잘렸는지" 를 판별 못 함. depth / source / status 분포도
표를 일일이 훑어야 보임. 외부 검수자에게 보낼 트리 HTML 에도 이 맥락이 빠져 있음.

### 설계 결정 (R9)

- **cap 도달 + sitemap 커버리지 + 분포** 세 묶음을 한 카드에. 각각 별도
  endpoint/카드로 분리할 정도로 큰 정보가 아님.
- **`discover_urls()` 반환을 `DiscoverResult` 데이터클래스로 wrap**.
  `__iter__` 가 기존 `(records, abort_reason)` 2-tuple unpacking 을 그대로
  지원해 16개 호출 site (대부분 테스트) 의 backward compat 유지. 새 호출자만
  `result.stats` 로 접근.
- **sitemap_total** 은 `_fetch_sitemap_seeds` 결과 길이 그대로 — 추가 fetch 없음.
- **cap_reached** 판정 — BFS 가 `len(results) < cfg.max_pages` 조건으로 자연
  종료하므로, 루프 후 `len(results) >= cfg.max_pages and queue` 로 보강.
  abort_reason 도 `"max_pages"` 로 명시.
- **분포** — 단순 dict 카운터, status 는 2xx/3xx/4xx/5xx/none 5 버킷으로 묶음.

### 변경 묶음 (R9)

#### 1. 통계 수집

- `DiscoverResult` 데이터클래스 — `records / abort_reason / stats` + `__iter__`.
- BFS 종료 후 `_compute_distribution(records)` 한 번 호출 + sitemap_total /
  cap_reached / abort_reason 합쳐 stats dict 생성.

#### 2. 메타 영속화 + 폴링 응답

- `_discover_worker` 가 `meta.json` 에 `stats` 키 추가.
- `DiscoverJob.stats: Optional[dict]` 필드 추가 → `/discover/{job_id}` 폴링
  응답에 자동 포함 (Pydantic 직렬화).

#### 3. UI 통계 카드

- `#discover-stats` 카드 — 결과 표 위, 트리 토글 위.
- 핵심 라인 + sitemap 라인 + depth/source/status mini-grid.
- 새 작업 시작 시 hidden + replaceChildren 으로 초기화.

#### 4. tree.html 헤더

- `render_self_contained_tree_html` 의 meta 에 stats 가 있으면 헤더 영역에
  한 줄 요약 — cap 경고 + sitemap 커버리지.

### 변경 파일 (R9)

| 파일 | 변경 |
| --- | --- |
| `shared/zero_touch_qa/url_discovery.py` | `DiscoverResult` 데이터클래스 + `__iter__`, `_compute_distribution`, sitemap_total / cap_reached 추적, `discover_urls` 반환 변경 |
| `recording-ui/recording_service/server.py` | `DiscoverJob.stats` 필드, `_discover_worker` 가 stats 를 meta.json + DiscoverJob 에 보존 |
| `recording-ui/recording_service/web/index.html` | `#discover-stats` 카드 |
| `recording-ui/recording_service/web/app.js` | `_renderDiscoverStats(stats)` + 폴링에서 호출, 새 작업 시 초기화 |
| `recording-ui/recording_service/web/style.css` | `.stats-card`, `.stats-row`, `.stats-cap-warn`, `.stats-mini-grid`, `.stats-bucket` |
| `recording-ui/recording_service/tree_builder.py` | `_render_stats_line` + 페이지 템플릿 슬롯 + `.cap-warn` CSS |

### 회귀 (R9)

- url_discovery 단위 24개 통과 (`__iter__` 가 2-tuple unpacking 호환 유지).
- `node --check` (app.js) / `ast.parse` 통과.
- 임의 fixture stats 로 tree.html 렌더 — `cap-warn` / `sitemap` / `stats-line`
  마커 모두 정상.
...
| 위험 | 회피 |
| --- | --- |
| `discover_urls` 반환 형식 변경 → 16개 호출 site 깸 | `DiscoverResult.__iter__` 가 `(records, abort_reason)` unpacking 호환. 모든 기존 호출이 깨짐 없이 동작 |
| sitemap 미존재 사이트의 sitemap_total | 빈 리스트면 None 유지 — UI sitemap 라인 자체 숨김 |
| cap_reached 가 max_depth 로 큐 비어 끝난 케이스를 cap 으로 오인 | `len(results) >= max_pages and queue` 두 조건 모두 — 큐 비면 cap 아님 |
| 분포 카운터 비용 | O(N) 단일 패스, 1000+ 결과에서도 ms 단위 |
...
#### 1. LLM 모드 — `shared/zero_touch_qa/executor.py`
...
#### 2. codegen 모드 — `recording-ui/recording_service/codegen_trace_wrapper.py`
...
#### 3. 사후 병합 — `recording-ui/recording_service/trace_parser.py`
...
| 파일 | 변경 |
| --- | --- |
| `shared/zero_touch_qa/executor.py` | `_extract_redirect_msg` 헬퍼, main 루프에서 호출 |
| `recording-ui/recording_service/codegen_trace_wrapper.py` | `_install_page_goto_redirect_capture`, sidecar flush 로직 |
| `recording-ui/recording_service/trace_parser.py` | `_read_redirects_sidecar`, parse_trace 에서 goto step 에 병합 |
