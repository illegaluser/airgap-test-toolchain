...
- `recording_service/tree_builder.py`
...
| 파일 | 변경 |
| --- | --- |
| `zero_touch_qa/url_discovery.py` | `DiscoveredUrl.parent_url`, BFS 큐 4-tuple 화, push/pop 갱신 |
| `recording_service/server.py` | `/discover/{id}/tree` + `/tree.html` 2 endpoint, CSV fieldnames |
| `recording_service/tree_builder.py` | (신규) 두 트리 빌더 + HTML 렌더 |
| `recording_service/web/index.html` | `#discover-tree-toggle` 카드 + 탭 + 다운로드 링크 |
| `recording_service/web/app.js` | `_renderDiscoverTrees(jobId)` + 노드 렌더 + 탭 전환 핸들러 |
| `recording_service/web/style.css` | `.tree-tabs`, `ul.tree`, `.tree-badge`, `.tree-pane` |
...
| 파일 | 변경 |
| --- | --- |
| `zero_touch_qa/url_discovery.py` | `DiscoverResult` 데이터클래스 + `__iter__`, `_compute_distribution`, sitemap_total / cap_reached 추적, `discover_urls` 반환 변경 |
| `recording_service/server.py` | `DiscoverJob.stats` 필드, `_discover_worker` 가 stats 를 meta.json + DiscoverJob 에 보존 |
| `recording_service/web/index.html` | `#discover-stats` 카드 |
| `recording_service/web/app.js` | `_renderDiscoverStats(stats)` + 폴링에서 호출, 새 작업 시 초기화 |
| `recording_service/web/style.css` | `.stats-card`, `.stats-row`, `.stats-cap-warn`, `.stats-mini-grid`, `.stats-bucket` |
| `recording_service/tree_builder.py` | `_render_stats_line` + 페이지 템플릿 슬롯 + `.cap-warn` CSS |
