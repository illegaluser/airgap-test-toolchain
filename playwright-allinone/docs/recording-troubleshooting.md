...
[recording-ui/recording_service/codegen_runner.py](../recording_service/codegen_runner.py) 변경:
...
grep -n "start_new_session\|killpg\|_close_pipes" recording-ui/recording_service/codegen_runner.py
...
[shared/zero_touch_qa/converter.py](../zero_touch_qa/converter.py) 의 라인 필터가 `page.` /
...
T-A (P0.4 / `shared/zero_touch_qa/converter_ast.py` 신설) 로 해소. AST 기반 converter
...
resolver 측 (`shared/zero_touch_qa/locator_resolver.py`) 도 `_split_modifiers` /
...
백엔드는 `recording-ui/recording_service/rplus/router.py` 모듈로 분리되어 `/experimental/sessions/{sid}/replay|enrich|compare` URL prefix 를 사용 — 코드 조직상 분리이지 실행 차단은 없음.
...
codegen 으로 녹화한 시나리오의 `page.frame_locator(...)` 체인은 [`converter_ast.py`](shared/zero_touch_qa/converter_ast.py) 의 `_segments_to_target` 가 자동으로 `frame=...` prefix 로 보존한다.
