...
`recording_service/replay_proxy.py` 가 컨테이너 Dify DB 에서 fresh token 을
...
- 신규 모듈 `zero_touch_qa/dify_token.py` — `fetch_token_from_container()`
...
- `Config.from_env()` — `DIFY_API_KEY` env 가 빈값이면 컨테이너 DB 조회 시도.
  ...
  기존 401 행동 호환).
- `recording_service/replay_proxy.py` 의 중복 함수 제거, 새 모듈 import 로
