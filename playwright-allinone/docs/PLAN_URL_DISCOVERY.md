...
내부적으로는 사람이 브라우저를 켜서 페이지 안의 링크를 따라가는 것과 같은 일을 자동으로 한다. 단, 화면을 띄우지 않고(headless) 빠르게 한다. 로그인이 필요한 사이트도 미리 저장해둔 **세션(쿠키 묶음)** 을 자동으로 끼워 넣어 들어간다 — 이건 최근에 추가된 기능이라 여기 그대로 빌려 쓴다. 1차 구현은 정적 anchor 링크 중심이다. SPA 라우터 버튼, hover 로만 열리는 메뉴, API 응답 안의 URL 까지 완전 수집하는 것은 후속 범위로 둔다.

## 이번에 만드는 범위와, 다음에 별도로 다룰 일

이번에 만드는 것: **URL 목록을 자동으로 수집해서 보여주고 파일로 저장하는 것**, 그리고 **사용자가 선택한 URL을 순서대로 방문하는 Python Playwright tour script 를 생성하는 것**까지.
...
그래서 이번 1차는 **"수집"** 과 **"선택 URL tour script 생성"** 까지, 그다음 별도 작업으로 **"수집된 목록을 받아서 자동 검증/회귀 판정"** 을 한다. 1차 결과 파일(CSV)과 tour script 결과(`tour_results.jsonl`)가 그대로 다음 작업의 입력이 되니 낭비는 없다.
...
- **URL 매칭은 정규화 후 set 비교**로 수행한다. `req.urls` 와 `urls.json` 양쪽 모두 `url_discovery.normalize_url()` 로 정규화한 뒤, 정규화된 입력 set 이 정규화된 발견 set 의 부분집합이 아니면 422. 응답 본문 / 생성 script 의 `URLS = [...]` 에는 클라이언트가 보낸 원본 형태가 아니라 `urls.json` 에 저장된 *원본 URL* 을 사용한다(매칭 후 룩업).
...
- 생성된 script 도 실행 시작 시 preflight verify 를 수행한다. 생성 시점 verify 와 실행 시점 사이에 세션이 만료될 수 있기 때문이다.
- 생성 파일은 `discoveries_root() / job_id / "tour_selected.py"` 에 저장한다.
- script 실행 결과는 기본적으로 같은 디렉토리 기준 `tour_results.jsonl` 과 `tour_screenshots/` 에 저장한다.
- 이 기능은 자동 검증/회귀 판정이 아니다. 선택 URL을 순서대로 방문해 status/title/final_url/error/screenshot 을 남기는 **tour smoke script 생성** 이다.

Preflight verify 규칙:
- auth_profile 이 없으면 preflight 는 생략한다.
- auth_profile 이 있으면 카탈로그의 `verify.service_url` 과 `verify.service_text` 를 script 에 상수로 기록한다.
- tour 시작 전 `VERIFY_SERVICE_URL` 로 이동한다.
- `VERIFY_SERVICE_TEXT` 가 있으면 body text 에 포함되는지 확인한다.
- `VERIFY_SERVICE_TEXT` 가 비어 있으면 응답 status `< 400` 이고 최종 host 가 검증 URL host 와 같거나 같은 도메인 계열인지 확인한다.
- 실패하면 URL tour 를 시작하지 않고 `tour_results.jsonl` 에 `{"phase": "preflight", "ok": false, "error": ...}` 한 줄만 남긴 뒤 exit code 2 로 종료한다.
- 성공하면 `{"phase": "preflight", "ok": true, ...}` 를 먼저 기록하고 URL tour 를 이어간다.
...
# 같은 머신/사용자 환경 실행 전제. 다른 머신에서는 즉시 fail.
# 공유 시 auth_profile 없이 재생성 권장.
STORAGE_STATE = "..."  # auth_profile 없으면 None. "~" prefix 가능.
CONTEXT_KWARGS_JSON = "..."  # JSON 직렬화된 fingerprint kwargs
CONTEXT_KWARGS = json.loads(CONTEXT_KWARGS_JSON)
HEADLESS = True
INCLUDE_SCREENSHOTS = True
PREFLIGHT_VERIFY = True
VERIFY_SERVICE_URL = "..."  # auth_profile 없으면 None
VERIFY_SERVICE_TEXT = "..."
...
10. URL 2~3개 선택 → "선택 URL Tour Script 생성" 클릭 → `tour_selected.py` 다운로드 및 결과 디렉토리 저장 확인.
11. 다운로드한 `tour_selected.py` 실행 → preflight 성공 후 선택 URL만 순서대로 방문하고 `tour_results.jsonl`, `tour_screenshots/` 생성 확인.
12. 만료된 storageState 로 `tour_selected.py` 실행 → URL tour 를 시작하지 않고 `tour_results.jsonl` 에 `phase=preflight, ok=false` 기록 후 exit code 2 확인.
13. 결과 파일 확인:
   ```
...
| tour script 를 검증 결과로 오해 | UI/문서에 tour script 는 방문 로그와 스크린샷을 남기는 smoke helper 이며 pass/fail 판정은 하지 않는다고 명시. |
...
- 발견된 URL 각각에 대한 자동 pass/fail 검증(콘솔/네트워크 에러 기준, 상태 코드 정책, baseline 비교). 1차 결과 CSV 와 tour 결과 jsonl 이 그대로 입력이 됨.
