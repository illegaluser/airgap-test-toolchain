...
| 2026-04-27 | Sprint 4B closure | Sprint 5 호스트 하이브리드 빌드 #9 (execute, `scenario_14.json` 14/14 PASS) + #14 (chat, 실 Dify `gemma4:26b` 12/12 PASS, Dify heal 2 회 성공) 가 chat E2E / mock_status / mock_data / 3단계 healing 의미적 핵심을 통합 검증해 4B 게이트 closure. 정량 fixture 회귀(S4B-01/02), `llm_sla.json` 집계 (S4B-03 후속), doc·convert smoke (S4B-05/06), 10 회 stability loop (S4B-11) 은 v4.1 출시 후 회귀 강화 backlog 로 이관 | §8.4 / §10.4.2 |
...
| `drag` | 요소를 마우스로 끌어서 다른 요소에 드롭 | `{"action": "drag", "target": "#item-1", "value": "#drop-zone"}` *(※ `value` 필드를 목적지 target으로 재활용)* | `locator(target).drag_to(page.locator(value))` |
...
에서 receiver-side resolver 의 합성 chain 해석이 활성됨 ([locator_resolver.py](shared/zero_touch_qa/locator_resolver.py)
...
이 modifier 들은 codegen AST 변환기 (`shared/zero_touch_qa/converter_ast.py`) 가
...
시나리오 단위 BrowserContext 분리는 이미 [executor.py:159-176](shared/zero_touch_qa/executor.py#L159-L176)
...
- **파일**: `shared/zero_touch_qa/__main__.py`, `executor.py`
- **작업 내용**:
...
- `shared/zero_touch_qa/__main__.py` 의 허용 액션 목록 변경
...
| ID | 작업 | 산출물 | 완료 기준 | 상태 |
| --- | --- | --- | --- | --- |
| S2-01 | `_VALID_ACTIONS`를 14대 DSL로 확장 | `shared/zero_touch_qa/__main__.py` | `upload`/`drag`/`scroll`/`mock_status`/`mock_data`가 구조 검증에서 거부되지 않음 | 완료 |
| S2-02 | 액션별 필수 필드 검증 추가 | `shared/zero_touch_qa/__main__.py` | `drag.value`, `upload.value`, `scroll.value=into_view`, `mock_status.value` 숫자성, `verify.condition` 화이트리스트, `mock_*.times` 양의 정수 검증 | 완료 |
| S2-03 | `_perform_action()`에 신규 5종 액션 구현 | `shared/zero_touch_qa/executor.py` | 신규 액션이 실제 Playwright API 호출로 실행됨 | 완료 |
| S2-04 | upload 경로 해석/방어 로직 추가 | `shared/zero_touch_qa/executor.py` | 허용 루트 밖 경로(`../`, 절대경로 탈출)가 차단되고 정상 파일만 업로드 가능 | 완료 |
| S2-05 | drag 목적지 사전 검증과 예외 메시지 정리 | `shared/zero_touch_qa/executor.py` | source/target 중 어느 쪽 실패인지 로그와 예외에 남음 | 완료 |
| S2-06 | scroll 계약 고정 | `shared/zero_touch_qa/__main__.py`, `executor.py` | `scroll.value`가 `into_view` 외 값이면 조기 실패 또는 normalize | 완료 |
| S2-07 | mock route 설치/수명주기 관리 | `shared/zero_touch_qa/executor.py` | `mock_*`가 기본 `times=1` 로 후속 스텝 오염을 막고, step.times 로 횟수 제어 가능. 의미를 본 계획서에 명문화 | 완료 |
| S2-08 | `verify.condition` 분기 구현 | `shared/zero_touch_qa/executor.py` | `visible`/`hidden`/`disabled`/`enabled`/`checked`/`value`/`text`(별칭 `contains_text`,`contains`) 가 각각 적절한 `expect()` 호출로 매핑 | 완료 |
| S2-09 | verify 실패 원인 분리 | `shared/zero_touch_qa/executor.py` | `VerificationAssertionError` 가 별도 클래스로 분리되어 locator 실패와 assertion 실패가 구분된다 | 완료 |
| S2-10 | Healer 병합 규칙 점검 | `shared/zero_touch_qa/executor.py` | `drag`/`upload` 는 LLM heal 의 `step.update` 로 target/value 모두 반영. `mock_*` 는 신규 `_execute_mock_step` 가 fallback URL 패턴 → Dify LLM 2단계 치유를 수행 | 완료 |
| S2-11 | `execute` 모드 호환성 확보 | `shared/zero_touch_qa/__main__.py`, `executor.py` | execute 모드에서도 `_validate_scenario` 가 호출되어 외부 scenario.json 에 신규 액션이 포함돼도 동일한 계약으로 실행됨 | 완료 |
...
| S2-15 | `regression_generator.py` 14대 확장 | `shared/zero_touch_qa/regression_generator.py` | upload/drag/scroll/mock_* 가 회귀 테스트 산출물에 정상 출력 (이전엔 빈 줄로 침묵 스킵) | 완료 |
...
- ✅ `chat`, `doc`, `execute` 모두에서 신규 5종 액션을 포함한 시나리오가 `_validate_scenario` 를 통과한다. execute 모드도 동일 검증을 거치도록 [`__main__.py:223`](shared/zero_touch_qa/__main__.py#L223) 가 보강되었다.
- ✅ `executor.py` 가 `upload`/`drag`/`scroll`/`mock_status`/`mock_data` 5종을 실제 Playwright API 호출로 수행한다 ([`executor.py:967-991`](shared/zero_touch_qa/executor.py#L967-L991), `_execute_mock_step`).
...
| S4A-03 | Dify 호출 metric 계측 | `shared/zero_touch_qa/dify_client.py`, `shared/zero_touch_qa/metrics.py`, `test/test_dify_metrics.py` | Planner/Healer 호출별 latency, timeout 여부, retry count, HTTP status, answer 길이를 `artifacts/llm_calls.jsonl` 로 기록 | 완료 |
...
| S4B-04 | chat E2E (실 Dify + agent) | Jenkins job 산출물 또는 `test/e2e/test_chat_flow.py` | `RUN_MODE=chat` → 실 Dify 호출 → staging 페이지 실행 → 산출물 7종 생성 → 종료코드 0 | 완료 — Build #14 chat 12/12 PASS (실 Dify `gemma4:26b`, fixture `full_dsl.html`) |
...
| S4B-09 | mock scope 안전장치 | `shared/zero_touch_qa/executor.py`, `test/test_mock_scope_guard.py` | 넓은 mock pattern 이 `TARGET_URL` 운영 host 또는 `MOCK_BLOCKED_HOSTS` 와 충돌하면 실패. `MOCK_OVERRIDE=1` 일 때만 명시 우회 | 완료 |
...
| ID | 작업 | 산출물 | 완료 기준 | 상태 |
| --- | --- | --- | --- | --- |
| S4C-01 | Convert 14대 parser 확장 | `shared/zero_touch_qa/converter.py` | `set_input_files` / `drag_to` / `scroll_into_view_if_needed` / `page.route` 를 upload / drag / scroll / mock_* 로 변환 | 완료 — converter.py 에 5 종 매핑 + `ast.literal_eval` 기반 mock_data body 평탄화 |
...
| S4C-04 | Zero Touch QA Report 운영 지표 섹션 | `shared/zero_touch_qa/report.py`, `test/test_report_metrics.py` | `index.html` 에 Planner/Healer 정확도, LLM SLA, heal 성공률, flake rate, pytest 결과 요약과 원본 metric 파일 링크가 표시됨. metric 파일이 없으면 섹션 생략 | 완료 |
...
| 4A-03a | Dify call metric writer 추가 | `shared/zero_touch_qa/metrics.py` | `python3 -m pytest test/test_dify_metrics.py -q` | `artifacts/llm_calls.jsonl` 에 JSON Lines append, artifacts_dir 없으면 no-op — 완료 |
| 4A-03b | `_call()` 에 metric 계측 연결 | `shared/zero_touch_qa/dify_client.py` | `python3 -m pytest test/test_dify_metrics.py -q` | planner/healer kind, elapsed_ms, retry_count, timeout, status_code, answer_chars, error 기록 — 완료 |
| 4A-03c | metric 집계 helper 추가 | `shared/zero_touch_qa/metrics.py` | `python3 -m pytest test/test_dify_metrics.py -q` | `llm_calls.jsonl` 에서 p50/p95/p99, timeout rate, retry total 산출 가능 — 완료 |
...
| 4B-03a | LLM SLA 집계 산출 | `shared/zero_touch_qa/metrics.py`, `test/e2e/test_llm_sla.py` | 단위 + 실 Dify gated pytest | `artifacts/llm_sla.json` 생성, baseline 과 threshold 를 분리 기록 |
...
| 4B-09a | mock scope guard 구현 | `shared/zero_touch_qa/executor.py`, `test/test_mock_scope_guard.py` | `python3 -m pytest test/test_mock_scope_guard.py test/test_mock_status.py test/test_mock_data.py -q` | blocked host / broad pattern 은 실패, `MOCK_OVERRIDE=1` 은 감사 로그와 함께 허용 — 완료 |
...
| 태스크 | 범위 | 구현 파일 | 검증 | 완료 조건 |
| --- | --- | --- | --- | --- |
| 4C-01a | converter parser 구조 정리 | `shared/zero_touch_qa/converter.py` | 기존 convert 테스트 | 기존 9대 변환 결과 변화 없음 |
| 4C-01b | upload/drag/scroll 변환 추가 | `shared/zero_touch_qa/converter.py` | `python3 -m pytest test/test_converter_14_actions.py -q` | `set_input_files`, `drag_to`, `scroll_into_view_if_needed` 가 각각 DSL 변환 |
| 4C-01c | page.route 변환 추가 | `shared/zero_touch_qa/converter.py` | converter 단위테스트 | `fulfill(status=...)` 는 `mock_status`, `fulfill(body=...)` 는 `mock_data` 변환 |
...
| 4C-04a | Zero Touch QA Report metric 섹션 렌더링 | `shared/zero_touch_qa/report.py`, `test/test_report_metrics.py` | `python3 -m pytest test/test_report_metrics.py -q` | metric JSON/JSONL 이 있으면 운영 지표 요약과 파일 링크 표시, 없으면 기존 report 유지 — 완료 |
...
- ✅ 4B 게이트 PASS — Sprint 5 호스트 하이브리드 빌드 #9 (execute, `scenario_14.json` 14/14 PASS) + #14 (chat, 실 Dify `gemma4:26b` 12/12 PASS, Dify heal 2 회 성공) 가 chat E2E + mock_status + mock_data + 3 단계 healing 의미적 핵심을 통합 검증.
...
- **chat**: 실 Dify Brain (`gemma4:26b`) Planner 호출. Sprint 5 §10.5 의 의미 압축 (LLM 이 14 항목 SRS 를 6 step 으로 통합) 동작 재확인 — chat 은 best-effort 의미 시연, 결정론 14 액션 검증은 execute 가 책임 (Sprint 5 §10.2 결정 재검증).
...
**발견된 결함과 영구 수정**: `build.sh` / `provision.sh` 의 `OLLAMA_MODEL` 기본값이 `gemma4:e4b` 인 반면 `dify-chatflow.yaml` 은 Sprint 5 §10.2 에서 `gemma4:26b` 로 고정. Fresh build 시 Dify Ollama provider 가 e4b 만 등록 → chat-messages 호출 시 `Model gemma4:26b not exist` HTTP 400. 양 스크립트의 기본값을 `gemma4:26b` 로 영구 수정해 향후 fresh rebuild 의 chat 모드 일관성 확보 (별도 운영 workaround 불필요).
...
#### 10.3.1 LLM 응답 처리 견고화 (`shared/zero_touch_qa/utils.py`, `shared/zero_touch_qa/__main__.py`)
...
#### 10.3.2 Executor multi-strategy chain — A 단 (`shared/zero_touch_qa/executor.py`)
...
#### 10.3.3 Healer 권한 확장 + strategy_trace 주입 — B 단 (`shared/zero_touch_qa/dify_client.py`, `shared/zero_touch_qa/executor.py`, `dify-chatflow.yaml`)
...
#### 10.3.6 리포트 첨부 섹션 정리 (`shared/zero_touch_qa/report.py`)
...
- 변경 파일: `Dockerfile`, `nginx.conf`, `ZeroTouch-QA.jenkinsPipeline`, `dify-chatflow.yaml`, `shared/zero_touch_qa/{__main__,executor,dify_client,utils,report}.py`, `test/test_sprint2_runtime.py`, `test/test_verify_conditions.py`.

## 11. Sprint 6 — chat 모드 결정론 달성 (2026-04-27)

### 11.1 배경

Sprint 5 §10.5 는 chat 모드를 "best-effort 의미 시연" 으로 위치시키고 결정적 14 액션 검증은 execute 모드에 책임 분담했다. 이는 당시 `gemma4:26b` 가 14 항목 SRS 를 자율 압축하는 한계를 받아들인 결정이었다. 그러나 v4.1 실환경 출시 검증 (§9 시퀀스) 중 사용자 트리거 chat 빌드에서 다음 결함이 추가로 노출됐다:
...
#### 11.3.1 Sanitizer 회복 (`shared/zero_touch_qa/__main__.py`)
...
#### 11.3.2 Healer action 화이트리스트 (`shared/zero_touch_qa/executor.py`, `dify-chatflow.yaml`)
...
#### 11.3.5 Press heuristic 범위 좁히기 (`shared/zero_touch_qa/executor.py`)
...
ZeroTouch-QA job, RUN_MODE=chat, gemma4:26b, 동일 fixture (`http://localhost:18081/fixtures/full_dsl.html`) 기준:
...
- gemma4:26b 가 thinking 텍스트를 외부에 출력하면서 `max_tokens=4096` 한도 초과.
...
- 변경 파일:
  - `shared/zero_touch_qa/__main__.py` — sanitizer leading token 회복, `--convert-only` 오용 early guard.
  - `shared/zero_touch_qa/executor.py` — `_HEAL_ACTION_TRANSITIONS`, `_is_allowed_action_transition`, heal handler 화이트리스트, press heuristic 범위.
...
## 12. 측정 액션 트랙 (Sprint 6 — 외부 평가 후속, 2026-05-13~)

본 섹션은 2026-05-13 의 외부 평가에서 식별된 **약점 2 (외부 SUT 측정 불가)** 를 해소하는 별 트랙이다. 평가 결과 14-DSL 이 임의 웹사이트 동작의 *약 45~55%* 만 표현 가능하며, **성능 / 시각 / 접근성 / Dialog / WebSocket / 클립보드 / IndexedDB read / Cookie verify** 영역은 0~10% 커버. 외부 SUT 벤치마크(트랙 2 Phase B2)를 시작하기 전에 측정 액션 5~7개를 선행 도입한다.

### 12.1 의사결정 로그

| # | 결정 | 일자 | 근거 |
| --- | --- | --- | --- |
| S6-D1 | 트랙 2 reframe = 옵션 B (측정 액션 확장 선행) | 2026-05-13 | 평가 결과 0% 커버 영역(성능/시각/접근성/Dialog)은 *암묵적 빠짐* — 의도적 OUT 아님. 도구 가치 확장 기회. |
| S6-D2 | 필수 5개 (performance / visual_diff / dialog_choose / storage_read / cookie_verify) + 권장 2개 (a11y_check / viewport_resize) | 2026-05-13 | Phase 0 §0.5 의 *암묵적 빠짐 10개* 중 ROI 큰 순서. WebSocket/SSE/right-click/클립보드 등은 Sprint 7 이후. |
| S6-D3 | 각 액션은 *별 PR* 로 main 머지, `feat/external-bench` 가 rebase | 2026-05-13 | 작은 surface area, 회귀 영향 격리. PR 한 개 거절돼도 다른 액션 진행 가능. |
| S6-D4 | LLM Planner 가 emit 하지 *않음* — 사용자 시나리오에만 등장 | 2026-05-13 | 14 standard actions 의 보조 액션 정책 (auth_login / reset_state 와 동일). dify-chatflow.yaml 갱신 불필요. |

### 12.2 필수 5개 액션 — 명세

| 액션 | target | value | 부작용 | 검증 대상 |
| --- | --- | --- | --- | --- |
| **dialog_choose** | `any` / `alert` / `confirm` / `prompt` / `beforeunload` | `accept` / `dismiss` / prompt 응답 텍스트 | 다음 dialog 의 응답을 미리 등록 (one-shot) | 등록 자체는 즉시 PASS, 매치 응답은 후속 step 에서 |
| **performance** | "" | 임계값 ms (예 `3000`) 또는 `FCP=2000,LCP=4000` 식 | `performance.timing` / Performance Observer 측정 | 임계값 이하면 PASS |
| **visual_diff** | "" | 기준 이미지 경로 (예 `fixtures/golden/home.png`) | 현재 viewport screenshot ↔ 기준 비교 (pixelmatch/PIL) | 차이 픽셀 % 임계 이하면 PASS |
| **storage_read** | localStorage/sessionStorage key | 기대 값 (정확 일치 또는 contains) | `page.evaluate("localStorage.getItem(...)")` | 값 일치 시 PASS |
| **cookie_verify** | cookie name (선택 `=domain`) | 기대 값 (없으면 *존재만* 검증) | `context.cookies()` 비교 | 일치/존재 시 PASS |

### 12.3 진행 상태 (2026-05-13)

5개 필수 액션 모두 한 브랜치(`feat/dsl-action-dialog-choose`)에 누적 커밋 — 사용자 검토 단일 사이클.

| 액션 | 상태 | 테스트 |
| --- | --- | --- |
| dialog_choose | **완료** | 8/8 (unit 2 + 통합 6) |
| storage_read | **완료** | 8/8 (unit 2 + 통합 6) |
| cookie_verify | **완료** | 8/8 (unit only — page.context.cookies() monkeypatch) |
| performance | **완료** | 5/5 (unit 2 + 통합 3) |
| visual_diff | **완료** | 7/7 (unit 3 + 통합 4) |

전체 36/36 PASS + 인접 회귀 영향 없음.

### 12.4 dialog_choose 구현 메모

- `QAExecutor._dialog_choice: Optional[tuple[str,str]]` 인스턴스 변수가 *다음 한 번* 의 응답을 보관.
- `_execute_inproc` 의 `_on_dialog` closure 가 매치 시 응답 적용 + None 으로 리셋 (one-shot).
- type 필터 `any` 면 모든 dialog 매치. 그 외는 `dlg.type` 정확 일치.
- 매치 안 되면 기존 동작(기본 dismiss + buffer 캡처) 유지 — 회귀 안전.
- `prompt` 응답 텍스트는 `dlg.accept(text)` 로 전달. `accept` / `dismiss` 가 아닌 임의 문자열은 prompt 응답으로 처리.
- 시나리오 validator (`__main__._VALID_ACTIONS` + `_TARGET_OPTIONAL_ACTIONS`) 에 등록 — target/value 없어도 통과 (기본값 `any`/`dismiss`).

### 12.5 잔여 한계 (Sprint 6)

- **WebSocket / SSE 측정** — 별 Sprint 7. Playwright `page.route` 가 HTTP 만 가로채는 한계.
- **클립보드 / Right-click context menu** — 별 Sprint 7. 사용 빈도 낮음.
- **모바일 gesture** — 별 트랙 (§8.7 Sprint 4 backlog 참조).
- **WebAuthn / Passkey** — [PLAN_PRODUCTION_READINESS.md](PLAN_PRODUCTION_READINESS.md) P0.1 의 OUT 그대로.
