...
9. [📦 모니터링 번들 다운로드 (다른 PC 에서 자동 실행)](#9--모니터링-번들-다운로드-다른-pc-에서-자동-실행)
...
## 9. 📦 모니터링 번들 다운로드 (다른 PC 에서 자동 실행)

### 언제 쓰는가

녹화한 시나리오를 **다른 PC** 에서 무인으로 반복 실행하고 싶을 때. 받는 쪽 PC 에는 [Replay UI](replay-ui-guide.md) 가 설치되어 있어야 한다.

### 어디 있는 버튼인가

`📊 결과 확인 및 스텝 추가` 섹션 → `Original Script` 카드 → 카드 우상단 액션 그룹의 **`[📦 모니터링 번들 다운로드]`** (`[📥 다운로드]` `[🔁 회귀 테스트]` 옆).

### 흐름

1. `[📦 모니터링 번들 다운로드]` 클릭 → 모달이 뜬다.
2. 입력 3개:
   - **로그인 프로파일 이름** — 받는 PC 의 Replay UI 에 등록할 이름과 정확히 같아야 한다 (대소문자 구분). 예 — `dpg-qa`. 받는 쪽 운영자와 미리 합의한 값을 적는다.
   - **로그인 상태 확인 URL** — 자동으로 추정값이 미리 채워져 있다. **이 값이 *이미 로그인된 페이지* 인지** 검토. 로그인 페이지 (`/login`, `/signin` 등) 가 들어가면 받는 쪽에서 매번 만료로 잘못 판정한다.
   - **사용할 스크립트** — 드롭다운. 보통 두 가지 중 선택:
     - `original.py (codegen 원본)` — 사람이 클릭한 그대로
     - `regression_test.py (LLM 적용)` — Play with LLM 으로 selector 가 강화된 버전. 권장.
3. 로그인 프로파일을 적용하지 않고 녹화한 세션이라 평문 비밀번호가 의심되면 **보안 경고 모달** 이 추가로 뜬다 — sanitize 후의 변경 라인 diff 를 보여주고 `[✗ 중단]` / `[✓ 동의하고 다운로드]` 선택을 묻는다. 의심 없는 깨끗한 세션은 이 모달이 안 뜬다.
4. `<세션ID>.bundle.zip` 이 다운로드된다.

### zip 안 구조 (참고)

```
<세션ID>.bundle.zip
├── script.py                ← 위에서 선택한 .py
├── scenario.json            ← 14-DSL (스텝 이름·스크린샷 라벨링용)
├── scenario.healed.json     (있을 때만)
├── metadata.json            ← 로그인 프로파일 이름 + 로그인 상태 확인 URL + 스크립트 출처
└── README.txt               ← 받는 쪽 안내
```

zip 안에 절대 안 들어가는 것:

- 로그인 상태 파일 (`*.storage.json`) — 받는 PC 에서 사람이 직접 로그인해야 한다
- 호스트 절대경로
- 평문 자격증명 (적극 sanitize + 잔존 시 422 또는 사용자 동의 필수)

### 받는 쪽 (모니터링 PC) 에서 무엇을 하나

받는 쪽 흐름은 [replay-ui-guide.md](replay-ui-guide.md) 에 단계별로 정리되어 있다. 요약:

1. Replay UI 에서 위에서 정한 이름과 똑같이 로그인 프로파일을 등록 (사람이 직접 1회 로그인)
2. 받은 zip 을 업로드
3. 실행

### 명령줄 (CLI) 진입점

UI 없이 같은 zip 을 만들 수 있다. CI 자동화에 쓴다.

```bash
python -m recording_service.recording_tools pack-bundle <세션ID> \
    --alias <프로파일이름> \
    --verify-url <로그인 상태 확인 URL> \
    --script-source regression_test.py \
    --out <받을 경로.zip>
```

(스위치 이름 `--alias` 는 내부 식별자다. 사용자 입장에서는 "로그인 프로파일 이름" 으로 읽으면 된다.)
...
| 0 | 성공 |
| 2 | 세션 디렉토리 없음 |
| 3 | 평문 자격증명 감지됐는데 동의 옵션 없음 (`--consent-plain-pw` 추가 필요) |
| 4 | 그 외 (스크립트 모호 / 메타데이터 누락 등) |
