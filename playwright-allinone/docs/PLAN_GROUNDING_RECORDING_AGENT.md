...
| 사용 모델 | `qwen3.5:9b` (도구 호출 신뢰성 미검증) |
...
few-shot 예시 3종을 시스템 프롬프트에 임베드한다 (qwen3.5:9b 가 IEEE 829 형식 지식 부족).
...
> **한 줄 요약**: `qwen3.5:9b` 가 Phase 2 의 도구 호출 에이전트를 구동할 수 있는지 검증한다. 불가하면 대체 모델을 식별한다.

### 목표

`qwen3.5:9b` 가 Phase 2 의 다중턴 도구 호출 에이전트 루프를 신뢰성 있게 구동할 수 있는지 검증한다. 불가하면 대체 모델을 식별하고 인프라 영향을 산정한다.
...
| --- | --- | --- | --- | --- |
| `qwen3.5:9b` | 베이스라인 | ~17GB | (T0.2 결과 기재) | 현재 |
...
**백엔드 (recording_service/server.py)**:
...
**프론트 (recording_service/web/index.html, app.js)**:
...
| 파일 | 변경 |
|---|---|
| `recording_service/server.py` | `?download=1` 쿼리 처리 + 기존 `/scenario` 도 동일 패턴, `_is_rplus_enabled()` 함수 제거 (또는 유지하되 healthz 만 노출), include_router 무조건 |
| `recording_service/server.py` | `GET /recording/sessions/{sid}/original` 신규 엔드포인트 |
| `recording_service/rplus/router.py` | `_require_rplus_enabled` Depends 제거, `is_rplus_enabled()` 도 제거 (옵션 A) |
| `recording_service/web/index.html` | 결과 패널에 탭 토글 + 다운로드 버튼 마크업 |
| `recording_service/web/app.js` | `getSessionOriginal()`, 탭 전환 핸들러, 다운로드 링크 갱신, `_isRplusEnabled()` 제거 (옵션 A) |
| `recording_service/web/style.css` | 탭 토글 스타일 (선택) |
...
| 영역 | 변경 |
|---|---|
| `recording_service/server.py` | `GET /recording/sessions/{sid}/original` 신규, 기존 `/scenario` 에 `?download=1` 쿼리 추가 (FileResponse + Content-Disposition). 게이트 코드 (`_is_rplus_enabled`, `import os`) 제거. `HealthResp` 의 `rplus_enabled` 필드 제거. R-Plus router 무조건 include. |
| `recording_service/rplus/router.py` | `_require_rplus_enabled` Depends + `is_rplus_enabled` 함수 제거. `import os` 제거. |
| `recording_service/web/index.html` | 결과 섹션 분리 — 메타용 `#result-section`, 본문용 신규 `#scenario-card` / `#original-card` 두 카드. 각 카드 헤더에 다운로드 링크 (`?download=1`). R-Plus 섹션 주석에서 게이트 조건 제거. |
| `recording_service/web/app.js` | `getSessionOriginal()` 신규. `_isRplusEnabled()` / `_lastHealth` 캐시 제거. `openSession()` 가 두 카드 fetch + `state=done` 일 때 R-Plus 항상 노출. start/delete 시 신규 카드 hide 처리. |
| `recording_service/web/style.css` | `.dl-link` 추가. |
