...
| Executor 모델 | `shared/zero_touch_qa/executor.py` per-test 호출 (데몬 아님) |
...
| `shared/zero_touch_qa/__main__.py` | `--convert-only` 플래그 추가 (T0.3 §"신규 플래그" 참조) | R-MVP TR.3 컨테이너 CLI 위임의 선결 조건 |
...
| 파일 | 역할 | 추정 LOC |
| --- | --- | --- |
| `shared/zero_touch_qa/grounding/__init__.py` | 진입점 | 30 |
| `shared/zero_touch_qa/grounding/extractor.py` | Playwright 로드 + a11y 추출 | 120 |
| `shared/zero_touch_qa/grounding/pruner.py` | 가지치기 룰 | 80 |
| `shared/zero_touch_qa/grounding/serializer.py` | LLM 친화적 직렬화 | 60 |
| `shared/zero_touch_qa/grounding/budget.py` | 토큰 예산 가드 | 40 |
...
1. `shared/zero_touch_qa/grounding/` 모듈 (~330 LOC)
...
| 위치 | 파일 | 역할 | 추정 LOC |
| --- | --- | --- | --- |
| 호스트 에이전트 | `recording-ui/recording_service/server.py` | FastAPI 엔드포인트 | 150 |
| 호스트 에이전트 | `recording-ui/recording_service/codegen_runner.py` | subprocess 래퍼 + 수명주기 | 120 |
| 호스트 에이전트 | `recording-ui/recording_service/session.py` | 세션 ID·상태 관리 | 80 |
| 호스트 에이전트 | `recording-ui/recording_service/storage.py` | 영속화 (recordings/) | 60 |
| 호스트 에이전트 | `recording-ui/recording_service/web/index.html` | 단일 페이지 UI | 200 |
| 호스트 에이전트 | `recording-ui/recording_service/web/app.js` | UI 로직 (vanilla) | 200 |
| 컨테이너 (재사용) | `shared/zero_touch_qa/converter.py` | 기존 변환기 (변경 최소) | — |
| 컨테이너 | `shared/zero_touch_qa/recording/enricher.py` | 시나리오 A 구현 | 100 |
| 컨테이너 | `shared/zero_touch_qa/recording/comparator.py` | 시나리오 B 구현 | 180 |
...
R-Plus 는 **백엔드만 분리** (`/experimental/*` URL prefix + 별도 `recording-ui/recording_service/rplus/` 모듈) — UI 는 운영자가 한 화면에서 녹화 → 검토 → 재실행/역추정/비교까지 흐름을 끊지 않도록 메인 결과 화면에 함께 둔다. 평가 게이트 (rubric / 5-sample / 90% replay) 미통과 상태에서는 환경변수 미설정으로 백엔드가 404 를 반환하고 프론트는 섹션 자체를 숨겨 노출 0 을 유지한다. 분리 의사결정 상세는 `PLAN_PRODUCTION_READINESS.md` §"P0.1 리뷰 후속 패치 (#5)" 참조.
...
1. `recording-ui/recording_service/` 모듈 (호스트, ~500 LOC + 테스트 — comparator/enricher 제외)
...
1. `shared/zero_touch_qa/recording/` 모듈 (컨테이너, enricher / comparator, ~280 LOC)
2. 결합 시나리오 A·B 구현
...
| 파일 | 역할 | 추정 LOC |
| --- | --- | --- |
| `shared/zero_touch_qa/agent/__init__.py` | 진입점 | 30 |
| `shared/zero_touch_qa/agent/server.py` | FastAPI 엔드포인트 | 120 |
| `shared/zero_touch_qa/agent/loop.py` | 에이전트 메인 루프 | 200 |
| `shared/zero_touch_qa/agent/tools/navigate.py` | 도구 구현 | 60 |
| `shared/zero_touch_qa/agent/tools/dom.py` | DOM 인벤토리 (Phase 1 재사용) | 40 |
| `shared/zero_touch_qa/agent/tools/interact.py` | 인터랙션 도구 | 100 |
| `shared/zero_touch_qa/agent/tools/verify.py` | 검증 도구 | 80 |
| `shared/zero_touch_qa/agent/tools/done.py` | 종료 + DSL 추출 | 60 |
| `shared/zero_touch_qa/agent/state.py` | 브라우저·세션 상태 관리 | 100 |
| `shared/zero_touch_qa/agent/safety.py` | 가드 (limits·timeouts) | 80 |
| `shared/zero_touch_qa/agent/llm_client.py` | Ollama 도구 호출 래퍼 | 100 |
| `shared/zero_touch_qa/agent/dsl_emitter.py` | 상태 → DSL 변환 | 120 |
...
1. `shared/zero_touch_qa/agent/` 모듈 (~1500 LOC + 테스트 400)
...
| `shared/zero_touch_qa/grounding/` | — | **신규** | 재사용 (옵션) | 재사용 | 재사용 | — |
| `shared/zero_touch_qa/recording/` | — | — | — | **신규** (enricher, comparator) | — | — |
| `shared/zero_touch_qa/converter.py` | — | — | 재사용 (변경 없음) | — | — | — |
| `shared/zero_touch_qa/__main__.py` | **수정 (`--convert-only` 플래그 추가, T0.2 PR)** | — | 호출만 | — | — | — |
| `shared/zero_touch_qa/agent/` | — | — | — | — | **신규** | 확장 |
| `recording-ui/recording_service/` (host) | — | — | **신규** (codegen 래퍼 + UI + 영속화) | 확장 (replay/enrich/compare) | — | — |
...
## TR.4+ — Recording Service UI 운영 보강 *(2026-04-29 추가, 검토 후 착수)*

R-MVP TR.4 의 결과 패널을 사용해 본 운영자 피드백:

> 1. 녹화 완료된 스크립트(.py 원본) 열람 기능 부재
> 2. 시나리오 파일(scenario.json) 다운로드 버튼 부재
> 3. 녹화 스크립트(.py 원본) 다운로드 버튼 부재
> 4. 녹화된 스크립트 재현(replay) 기능이 메인 화면에서 안 보임
> 5. R-Plus 영역 자체가 화면에 노출 안 됨 (RPLUS_ENABLED 미설정 + host daemon 환경변수 주입 누락)

### TR.4+.1 — `original.py` 열람/다운로드

**백엔드 (recording-ui/recording_service/server.py)**:

```python
GET /recording/sessions/{sid}/original
  ?download=1   # 첨부 다운로드 (Content-Disposition: attachment)
  └ 미설정      # text/x-python 본문 그대로 반환 (브라우저 표시용)
```

- 응답 코드: 200 (파일 있음) / 404 (세션 없음 또는 original.py 없음)
- `state ∈ {recording, done, error}` 모두에서 동작 — 변환 실패해도 원본은 봐야 한다.
- 파일 경로: `storage.original_py_path(sid)`.

**프론트 (recording-ui/recording_service/web/index.html, app.js)**:

결과 패널의 `<pre id="result-json">` 위에 **탭 토글** 2개:

```
[ 📄 Scenario JSON  ⬇ ] [ 🐍 Original .py  ⬇ ]
```

- 탭 클릭 시 대응 fetch 후 동일 `<pre>` 에 본문 표시.
- 각 탭 옆 `⬇` 는 `<a download="<sid>-scenario.json" href="...">` / `<a download="<sid>-original.py" href="...?download=1">`.
- 디폴트 탭: Scenario JSON (state=done) / Original .py (state=error 또는 변환 실패).

### TR.4+.2 — `scenario.json` 다운로드

기존 `GET /recording/sessions/{sid}/scenario` 엔드포인트는 본문 반환 중. 백엔드 변경 없음. 프론트의 다운로드 버튼만 TR.4+.1 의 탭 옆에 함께 배치 — `?download=1` 쿼리 옵션을 같은 엔드포인트에도 추가해 일관성 (또는 `Blob` 로 클라이언트 측 다운로드).

**선택지**:
- (a) 서버 측 `?download=1` 분기 추가 (Content-Disposition 헤더). 일관성↑, 코드 +5라인.
- (b) 프론트 `Blob` + `URL.createObjectURL` + 가짜 클릭. 서버 변경 0, 코드 +10라인.

→ 권장: (a). scenario / original 두 엔드포인트 모두 같은 패턴.

### TR.4+.3 — 녹화 스크립트 재현(replay) — R-Plus 와 일치

사용자 요구의 "녹화된 스크립트 재현" 은 R-Plus 의 `POST /experimental/sessions/{sid}/replay` 와 **동일 기능**. 핵심 이슈는 ④/⑤ — 메인 UI 에서 R-Plus 섹션이 보이지 않는다는 것.

→ TR.4+.4 의 게이트 정책 결정으로 통합 처리.

### TR.4+.4 — R-Plus 게이트 정책 결정 *(검토 필요)*

현 상태:
- 코드: `RPLUS_ENABLED=1` 미설정 시 백엔드 `/experimental/*` 전부 404, 프론트 R-Plus 섹션 hidden.
- 운영: `mac-agent-setup.sh` 가 host daemon 띄울 때 환경변수 주입 안 함 → 항상 비활성.

선택지 3가지:

| 옵션 | 변경 | 효과 | 트레이드오프 |
|---|---|---|---|
| (A) 게이트 폐기 | `_require_rplus_enabled` Depends 제거, 프론트 `_isRplusEnabled()` 항상 true | R-Plus 가 항상 메인 화면에 노출. 운영 단순화. | 평가 미통과 상태에서도 사용자가 호출 → docker exec / Ollama 의존성 실패 가능 (도구 자체는 동작) |
| (B) 게이트 유지 + setup 스크립트가 ON 으로 주입 | `mac-agent-setup.sh` / `wsl-agent-setup.sh` 가 launchd plist / systemd unit 에 `RPLUS_ENABLED=1` 추가 | R-Plus 가 기본 ON. 평가 미통과 운영자는 명시적으로 OFF 해야. | 게이트 의도 (실험 분리) 약화 |
| (C) 게이트 유지 + UI 활성화 안내 | RPLUS=false 일 때도 R-Plus 섹션을 회색 disabled 상태로 표시 + 호버 시 "활성화 명령 안내" 툴팁 | 사용자가 기능 존재를 인지. 명시 opt-in. | UI 클러터 |

**권장**: **(A) 게이트 폐기**. 사용자가 직전 대화에서 "R-Plus 는 일상 사용 빈도 높아서 메인에 함께 둠" 이라고 결정했고, 게이트는 평가 단계 분리를 위한 수단이었음. 이제 사용자가 정상 기능으로 다루겠다고 한 이상 게이트 자체가 모순. URL prefix `/experimental/` 는 보존 (코드 조직상 의미 있음) 하되 실행 차단은 풀자.

### 변경 파일 요약 (예상)

| 파일 | 변경 |
|---|---|
| `recording-ui/recording_service/server.py` | `?download=1` 쿼리 처리 + 기존 `/scenario` 도 동일 패턴, `_is_rplus_enabled()` 함수 제거 (또는 유지하되 healthz 만 노출), include_router 무조건 |
| `recording-ui/recording_service/server.py` | `GET /recording/sessions/{sid}/original` 신규 엔드포인트 |
| `recording-ui/recording_service/rplus/router.py` | `_require_rplus_enabled` Depends 제거, `is_rplus_enabled()` 도 제거 (옵션 A) |
| `recording-ui/recording_service/web/index.html` | 결과 패널에 탭 토글 + 다운로드 버튼 마크업 |
| `recording-ui/recording_service/web/app.js` | `getSessionOriginal()`, 탭 전환 핸들러, 다운로드 링크 갱신, `_isRplusEnabled()` 제거 (옵션 A) |
| `recording-ui/recording_service/web/style.css` | 탭 토글 스타일 (선택) |
| `test/test_recording_service.py` | `/original` 엔드포인트 회귀 (200/404, download 플래그), R-Plus 게이트 폐기 회귀 (옵션 A 채택 시 기존 `*_when_rplus_disabled` 테스트 갱신/제거) |
| `docs/recording-troubleshooting.md` | §8 R-Plus 활성화 절 갱신 — "기본 ON" 으로 정정 |

### 추정 시간

- TR.4+.1 (`/original` + 탭 UI): 1.5h
- TR.4+.2 (download 쿼리): 30m
- TR.4+.3 (재현 기능 — replay 와 동일이라 별도 0): 0
- TR.4+.4 (게이트 폐기): 1h (코드 30m + 테스트 30m)
- 총 ~3h

### 검토 결정 (2026-04-29 사용자)

1. **TR.4+.4** → **(A) 게이트 폐기**. "언제든 기능을 활용할 수 있어야 한다"
2. **다운로드** → **(a) 서버 `?download=1`**
3. **결과 표시** → **별도 카드 2개**
4. **R-Plus 섹션 위치** → **현행 유지** (결과 패널 아래 별도 섹션)

### 구현 결과 (2026-04-29)

| 영역 | 변경 |
|---|---|
| `recording-ui/recording_service/server.py` | `GET /recording/sessions/{sid}/original` 신규, 기존 `/scenario` 에 `?download=1` 쿼리 추가 (FileResponse + Content-Disposition). 게이트 코드 (`_is_rplus_enabled`, `import os`) 제거. `HealthResp` 의 `rplus_enabled` 필드 제거. R-Plus router 무조건 include. |
| `recording-ui/recording_service/rplus/router.py` | `_require_rplus_enabled` Depends + `is_rplus_enabled` 함수 제거. `import os` 제거. |
| `recording-ui/recording_service/web/index.html` | 결과 섹션 분리 — 메타용 `#result-section`, 본문용 신규 `#scenario-card` / `#original-card` 두 카드. 각 카드 헤더에 다운로드 링크 (`?download=1`). R-Plus 섹션 주석에서 게이트 조건 제거. |
| `recording-ui/recording_service/web/app.js` | `getSessionOriginal()` 신규. `_isRplusEnabled()` / `_lastHealth` 캐시 제거. `openSession()` 가 두 카드 fetch + `state=done` 일 때 R-Plus 항상 노출. start/delete 시 신규 카드 hide 처리. |
| `recording-ui/recording_service/web/style.css` | `.dl-link` 추가. |
| `test/test_recording_service.py` | 신규 회귀 5개 (`scenario?download=1`, `/original` 본문/다운로드/404), 게이트 차단 테스트 4개 제거, healthz `rplus_enabled` 필드 검사를 "필드 없음" 으로 반전, `rplus_on` fixture 는 시그니처 호환을 위해 no-op 으로 유지. |
| `docs/recording-troubleshooting.md` | §8 "R-Plus 게이트 폐기, 항상 활성" 으로 정정. §9 신규 — 다운로드/열람 사용법. |

전체 통과: `test/test_recording_service.py` 76 passed.
