...
8. 호스트 Replay UI(18093) 자동 기동 — agent-setup step 6.6 이 venv uvicorn 으로 띄움
...
이 스크립트는 JDK/Python 확인, venv 생성, Playwright Chromium 설치, Jenkins agent 연결, Recording UI(18092) + 호스트 Replay UI(18093) 두 데몬 기동까지 처리한다. 휴대용 Replay UI(18099) 는 받는 PC 가 zip 풀고 `Launch-ReplayUI.{bat,command}` 더블클릭하는 별도 흐름.
