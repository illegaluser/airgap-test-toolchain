...
| **시나리오 묶음** | 녹화 PC 가 만들어 보낸 zip 파일. 안에 Playwright 스크립트 + 시나리오 정의 + 메타데이터가 들어 있다. 파일명은 보통 `<세션ID>.bundle.zip` |
...
페이지 상단에 `🎬 Replay UI` 와 4개 카드 (`로그인 프로파일`, `시나리오 묶음`, `실행`, `결과`) 가 보이면 설치 OK.

> 이 주소는 **모니터링 PC 자기 자신에서만** 접속된다. 다른 PC 에서 LAN 으로 접속해도 거부된다 (의도된 보안 설정).

### 4.6 안 떴을 때 — 수동 기동

브라우저로 접속했는데 페이지가 안 뜨면 OS 가 startup task 를 안 띄운 상태다. 한 번 수동으로 기동해 본다.
...
& "$env:USERPROFILE\.dscore.ttc.monitor\venv\Scripts\python.exe" `
    -m uvicorn replay_service.server:app --host 127.0.0.1 --port 18094
```

콘솔에 `Uvicorn running on http://127.0.0.1:18094` 가 찍히면 OK. 다시 브라우저로 접속해 본다.
...
### 5.2 시나리오 묶음 받아오기 (녹화 PC 에서)

**왜 하는가** — 녹화 PC 가 만든 시나리오 zip 1개가 모니터링 PC 에서 1개 시나리오로 실행된다.

**녹화 PC 작업** ([recording-ui.md §9](recording-ui.md#9--모니터링-번들-다운로드-다른-pc-에서-자동-실행) 참조)

1. 녹화 PC 의 Recording UI 에서 시나리오 녹화를 끝낸다.
2. 결과 카드 → `Original Script` 카드 → `[📦 모니터링 번들 다운로드]` 클릭.
3. 모달에 **§5.1 에서 정한 프로파일 이름과 똑같이** (예 — `dpg-qa`) 입력. 다른 입력은 자동 제안값을 검토 후 확인.
4. `<세션ID>.bundle.zip` 파일이 다운로드된다.

이 zip 을 USB / 이메일 / 사내 공유 폴더 등으로 모니터링 PC 에 옮긴다.

### 5.3 시나리오 묶음 업로드

**왜 하는가** — Replay UI 가 zip 을 풀고 나중에 자동 실행하기 위해.

**절차**

1. `📦 시나리오 묶음` 카드의 `[⬆ 업로드]` 클릭
2. 받은 zip 파일 선택
...
4. 카드에 한 줄 추가됨:
   ...
   | 시나리오 | 프로파일 | 등록일 | 크기 | 액션 |
   |---|---|---|---|---|
   | dashboard-tour | dpg-qa | 방금 | 18KB | `[▶ 실행]` `[🗑]` |

> `[▶ 실행]` 버튼이 회색이고 마우스 올리면 *"로그인 프로파일이 등록되어 있지 않습니다"* 가 뜨면 §5.1 부터 다시 한다. 시나리오의 프로파일 이름과 §5.1 에서 등록한 이름이 정확히 일치해야 한다 (대소문자 구분).
...
```
crontab 에 다음을 추가:
*/30 * * * * ~/.dscore.ttc.monitor/venv/bin/python -m monitor replay \
   ~/.dscore.ttc.monitor/scenarios/dashboard-tour.bundle.zip \
   --out ~/.dscore.ttc.monitor/runs/auto
```
...
schtasks /create /sc minute /mo 30 /tn "Monitor Replay dashboard-tour" \
   /tr "%USERPROFILE%\.dscore.ttc.monitor\venv\Scripts\python.exe -m monitor replay \
        %USERPROFILE%\.dscore.ttc.monitor\scenarios\dashboard-tour.bundle.zip \
        --out %USERPROFILE%\.dscore.ttc.monitor\runs\auto"
```

이걸 시나리오 zip 1개당 한 번씩 등록한다 (시나리오마다 별도 항목).
...
| `python -m monitor replay <zip 경로> --out <결과 폴더>` | 시나리오 묶음 1회 실행 |
...
| 로그인 상태 (`*.storage.json`) | 모니터링 PC 의 보관함에만 있음. 시나리오 묶음 zip 안에는 절대로 안 들어감 |
| ID / 비밀번호 | 모니터링 PC 어디에도 평문으로 저장 안 함. 사람이 그때그때 브라우저에 직접 입력 |
| 시나리오 묶음 zip | 평문 비밀번호 / 호스트 절대경로 / 로그인 상태 파일 모두 미포함 (녹화 PC 가 zip 만들 때 자동 검사 + 차단) |
...
| 증상 | 원인 / 해결 |
|---|---|
| 브라우저에서 `http://127.0.0.1:18094` 가 안 뜸 | startup task 가 안 떠 있다. [§4.6 수동 기동](#46-안-떴을-때--수동-기동) 시도 → 그래도 안 뜨면 `~/.dscore.ttc.monitor/replay-ui.stderr.log` 확인 |
| 시나리오 행의 `[▶ 실행]` 이 회색 | 그 시나리오에 명시된 로그인 프로파일 이름이 모니터링 PC 에 등록 안 됨. tooltip 이 가리키는 이름으로 §5.1 부터 다시 |
| 로그인 했는데 매 30분마다 만료 알람 | 시나리오의 "로그인 상태 확인 URL" 이 잘못 — 로그인 페이지로 잡혀 있으면 항상 만료로 판정. 녹화 PC 에서 zip 다시 만들 때 *이미 로그인된 페이지* 로 그 URL 을 다시 입력 |
...
| 시나리오 묶음 zip 만들기 (녹화 PC 쪽) | [recording-ui.md](recording-ui.md) |
