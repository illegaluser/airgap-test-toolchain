...
| test_readme_contains_alias_and_verify_url | (보조) |
...
| test_run_bundle_alias_missing_in_catalog_exit_three | T17 의 사전 차단 |
...
- [ ] **T1** 녹화 PC → 모니터링 PC (Windows): alias 시드 후 `python -m monitor replay <bundle>` 실행 → step PASS / exit 0
- [ ] **T3** 만료 시뮬레이션 (storage 강제 삭제) → exit 3 + jsonl `auth_seed_expired`
- [ ] **T4** T3 후 Replay UI [↻ Re-seed] → 다시 실행 → step PASS / exit 0
- [ ] **T5** Replay UI 에서 bundle 업로드 → 실행 → SSE 실시간 jsonl + 결과 카드
...
- [ ] **T17** 시드 미완 alias 의 bundle 행 — [▶ 실행] 비활성 + tooltip "alias 시드 필요" + 412 사전 차단
- [ ] **T18** 동일 이름 bundle 재업로드 — 첫 시도 409 → confirm prompt → `?overwrite=1` 재시도 → 200
- [ ] **T19** 글로벌 알람 인디케이터 — 만료된 alias 가 N 개일 때 헤더에 `🔴 N 시드 만료` 표시, 카운트 정확
