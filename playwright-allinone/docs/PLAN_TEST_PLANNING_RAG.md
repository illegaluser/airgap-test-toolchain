...
- **신규 Dify 챗봇**: `Test Planning Brain` (별도 chatflow YAML).
  ...
  - LLM: `qwen3.5:9b` (기존과 동일 모델, 동일 provider).
...
| **모델 / 오프라인 반입 가이드 정합성** — README / Dockerfile / 오프라인 반입 list 가 모두 `qwen3.5:9b` + `bona/bge-m3-korean:latest` 로 일관 | grep `gemma4:e4b` / 오프라인 반입 list 검토 |
...
- **모델**: `qwen3.5:9b` (Ollama, 호스트, 기존과 동일).
...
| LLM 모델 | qwen3.5:9b | qwen3.5:9b | provider 공유 |
| Embedding | (사용 안 함) | bge-m3 | 신규 추가 |
...
| 옵션 | 값 | 안전 마진 (14942 기준) | qwen3.5:9b 추론 속도 영향 |
| --- | --- | --- | --- |
| 보수 | 16384 | ~1400 토큰 (10%) — 턱걸이 | -30% |
| **권장** | **20480** | **~5500 토큰 (27%)** | -50% |
| 안전 | 24576 | ~9600 토큰 (39%) | -65% |

`qwen3.5:9b` 의 native context 한계는 262144 (256k 토큰, Sprint 6 검증) 이라 모델 자체
...
| ID | 작업 | 측정 방법 | 합격선 |
| --- | --- | --- | --- |
| T-pre-13a | qwen3.5:9b 의 `scenario_dsl` 모드 결정성 사전 측정 | 신규 chatflow + 임시 KB 1개에 sample 문서 1건 업로드 → 동일 query 10회 호출 → 응답 첫 글자 `[` / 끝 `]` 비율 + `_validate_scenario` 통과율 측정 | ≥ 80% (90% 미달 시 LLM 노드 `structured_output` 옵션 + JSON schema 강제 필요 검토) |
...
| T-02a | KB 생성 시 indexing_technique=high-quality, embedding_model=bge-m3 명시 | `provision.sh` | T-02 안의 POST body | DB `datasets` 테이블 검증 |
...
| T-05e | LLM 노드 (qwen3.5:9b, max_tokens=8192, temp=0.1) | T-05 안 | LLM node JSON | provision.sh 의 model 등록과 일치 |
...
| 의존성 | 현재 상태 | 위험 | 완화 |
| --- | --- | --- | --- |
| `qwen3.5:9b` (Ollama) | 호스트에 설치됨 | 모델 변경 시 Sprint 6 보강 효과 무효화 가능 | README 사전 준비에 모델 명시 (T-01b) |
...
| LLM 모델 | `qwen3.5:9b` (기존 공유) | 2026-04-27 | provider 통합 운영 |
...
[17:06:58] [·] 2-3g. Workspace 기본 모델 설정 (llm=qwen3.5:9b, embedding=bona/bge-m3-korean:latest)
