...
[A 머신, 인터넷 ON]                                       ⌜ 한 줄 명령으로 ⌝
  ...
  3-1) 자산 다운로드   ←──────────── (위 명령으로 이미 완료)
                              💾 ~12GB
       ↓
  3-2) 통합 이미지 빌드 + 반입 패키지 산출 (bash scripts/build-{wsl2,mac}.sh)
       ↓                     ⏱ 빌드 30~90분 + 반출 10~20분 / 💾 ~25GB peak
       (3-2 가 자동으로 offline-prefetch.sh 까지 호출 — 외부망 작업의 자연스러운 종착지가
        반출 패키지이므로 단일 명령으로 통합. 로컬 시험만 원하면 --no-tarball 플래그)
...
```bash
# 1차 머신 (WSL2 / amd64) — 정본
bash scripts/build-wsl2.sh                # build + 반출 tarball 자동 생성
# bash scripts/build-wsl2.sh --no-tarball # 로컬 시험만, 반출 단계 생략
...
📦 **결과**:
- 로컬 daemon 의 `ttc-allinone:wsl2-dev` (또는 `mac-dev`) 이미지 (약 6~10 GB)
- `offline-assets/${ARCH}/` 의 반출 tarball 3개 (all-in-one + GitLab + Dify Sandbox) — `--no-tarball` 미지정 시
...
> jenkins-init 모두 단일 소스. 빌드 방식은 `docker build` (Docker 24+ 부터 BuildKit 이 기본,
> 29+ 부터는 `docker build` 자체가 `docker buildx build` 의 alias). buildx *멀티플랫폼 manifest 운영*
> 만 의도적으로 안 함 — Mac/WSL2 각각 native 빌드라 `--platform` cross-compile 오버헤드가 낭비.
...
> ℹ §3.3 의 `build-{wsl2,mac}.sh` 가 기본으로 *이 단계까지 자동 실행* 합니다 (`--no-tarball`
> 미지정 시). 따라서 별도 호출 불필요. 아래는 **build 와 분리해 다시 export 만 하고 싶을 때**
> 수동 명령:

**(1) 이미지 tarball export** *(수동 호출 — build 후 분리 export 시):*
