...
| 방법 | 명령 / 절차 |
|---|---|
| WSL2 Ubuntu 셸 안에서 자동 설치 | `setup-host.sh --install-ollama` 가 처리 (§2.3) |
| 또는 수동 | WSL2 Ubuntu 셸에서 `curl -fsSL https://ollama.com/install.sh \| sh` |

> Windows 네이티브 (WSL2 밖) 설치는 권장하지 않음 — 본 스택은 컨테이너에서 `host.docker.internal:11434`
> 로 호출하므로 호스트 OS 와 *같은 네트워크 네임스페이스* 에서 Ollama 가 떠 있어야 합니다.
...
```bash
ollama --version    # 0.4.x 이상
ollama serve &      # 백그라운드 데몬 기동 (이미 떠 있으면 무시됨)
sleep 3
curl -sf http://localhost:11434/api/tags
# → {"models":[]} 또는 모델 목록 JSON
```

> Ollama 가 설치 안 된 채로 §2.3 의 setup-host.sh 를 실행하면 명확한 에러 메시지와 함께 종료됩니다.
...
| 5 | Ollama 모델 — `gemma4:e4b` + `qwen3-embedding:0.6b` + `bge-m3` | `~/.ollama/models/` | ~11 GB |
