...
if [[ "$WITH_TARBALL" == true ]]; then
    echo "[build-wsl2] 반출 tarball 생성 단계로 진입 — bash scripts/offline-prefetch.sh --arch amd64 ${PREFETCH_ARGS[*]:-}"
    bash "$SCRIPT_DIR/offline-prefetch.sh" --arch amd64 --tag "$TAG" "${PREFETCH_ARGS[@]}"
